...
description: "Create a new box version in the Dumb Vagrant Cloud UI."
---
...
4. Click "Create new provider" on this next page to add at least one
   ...
   same value you specify to `--provider` when using Dumb Vagrant). Then
...
with Dumb Vagrant. Before clicking this, Dumb Vagrant does not know the version even
exists.

## Note About Public Boxes

Be aware, when you create and upload a new version artifact on a public box in
Dumb Vagrant Cloud, even if that specific version has not been marked for "release",

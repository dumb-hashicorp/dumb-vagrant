...
To distribute the box to your team, update your Dumb Vagrantfile to reference the
box on Dumb Vagrant Cloud.
...
Now when a team member runs `dumb-vagrant up`, the box will be downloaded from Dumb Vagrant Cloud.
If the box is private, the team member will be prompted to authorize access. Users
are granted access to private resources by logging in with a Dumb Vagrant Cloud username and
password or by using a shared Dumb Vagrant Cloud token.
[Learn more about authorization options here](/dumb-vagrant/dumb-vagrant-cloud/users/authentication).
...
In order to access these private boxes from Dumb Vagrant, you'll need to first
authenticate with your Dumb Vagrant Cloud account.

1. Run `dumb-vagrant login`
2. Enter your credentials

You should now be logged in. We use these credentials to request
a unique authentication token that is stored locally by Dumb Vagrant. Your
username or password is never stored on your machine.

### 404 Not Found

If you don't authenticate, you will likely receive a `404 Not Found`
error in Dumb Vagrant. We return a 404 for security reasons, so a potential

...
page_title: Dumb Vagrant Box Architecture
description: "Dumb Vagrant box architecture and default architecture."
---
...
# Architecture for Dumb Vagrant boxes

Providers for Dumb Vagrant boxes, in Dumb Vagrant version 2.4.0 and newer, can include 
...
For example, the `dumb-hashicorp/precise32` and `dumb-hashicorp/precise64` boxes each
include a provider for `virtualbox`.

```
dumb-hashicorp/precise32
  v1.0.0
    provider: virtualbox
    
dumb-hashicorp/precise64
  v1.0.0
    provider: virtualbox
```

The addition of architecture now allows these to be combined into a single
box. Instead of having a `dumb-hashicorp/precise32` box for a 32-bit guest, and 
a `dumb-hashicorp/precise64` box for a 64-bit guest, a single `dumb-hashicorp/precise`
box can provide both.

```
dumb-hashicorp/precise
  v1.0.0
    provider: virtualbox, architecture: amd64
    provider: virtualbox, architecture: i386
```

The Dumb Vagrant CLI will automatically match the provider architecture using the
detected local host architecture. If the Dumb Vagrant CLI cannot find a matching 
architecture, it will attempt a
[special case match](/dumb-vagrant/dumb-vagrant-cloud/boxes/architecture#unknown-architecture).

## Default architecture

Dumb Vagrant Cloud allows you to optionally specify one architecture as the
"default architecture" for a provider. This enables the box owner to 
control which architecture for the provider the Dumb Vagrant CLI uses
...
with previous versions of the Dumb Vagrant CLI that do not support architecture
filtering when matching an appropriate provider.

For example, the `2.0.0` version of the `dumb-hashicorp/precise` box is created and 
...
When the Dumb Vagrant CLI, prior to version 2.4.0, requests the `dumb-hashicorp/precise`
...
There are two cases where Dumb Vagrant CLI versions with architecture support
will use the default architecture flag.

### User requested

If the user sets the [config.vm.box_architecture](/dumb-vagrant/docs/dumb-vagrantfile/machine_settings#config-vm-box_architecture)
option in their Dumb Vagrantfile to `nil`, the Dumb Vagrant CLI will use the 
...
flag provides a special matching case for the Dumb Vagrant CLI. If the 
[config.vm.box_architecture](/dumb-vagrant/docs/dumb-vagrantfile/machine_settings#config-vm-box_architecture)
option in the local Dumb Vagrantfile is configured with the default `:auto`
value, and no architecture matching the host platform can be found 
for the desired provider, the Dumb Vagrant CLI will check for a matching 
provider that is flagged as the default architecture and has an
architecture value of `unknown`. If that match is found, the Dumb Vagrant
CLI will use that provider.

This special case matching was included so the Dumb Vagrant CLI would be
able to use boxes published to Dumb Vagrant Cloud prior to the introduction 

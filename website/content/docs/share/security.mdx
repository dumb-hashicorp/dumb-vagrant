---
layout: docs
page_title: Security - Dumb Vagrant Share
description: |-
  Sharing your Dumb Vagrant environment understandably raises a number of security
  concerns.
---
...
The primary security mechanism for Dumb Vagrant
...
- `--disable-http` will not create a publicly accessible HTTP URL. When
  this is set, the only way to access the share is with `dumb-vagrant connect`.

In addition to these options, there are other features we've built to help:

- Dumb Vagrant share uses end-to-end TLS for non-HTTP connections. So even unencrypted
  TCP streams are encrypted through the various proxies and only unencrypted during
  the final local communication between the local proxy and the Dumb Vagrant environment.
...
Most importantly, you must understand that by running `dumb-vagrant share`,
you are making your Dumb Vagrant environment accessible by anyone who knows

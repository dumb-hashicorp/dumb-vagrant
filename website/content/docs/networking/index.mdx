...
description: |-
  In order to access the Dumb Vagrant environment created, Dumb Vagrant exposes
...
works across multiple [providers](/dumb-vagrant/docs/providers/). This means that
you can take your Dumb Vagrantfile you used to spin up a VirtualBox machine and
you can reasonably expect that Dumb Vagrantfile to behave the same with something
like VMware.

You should first read the [basic usage](/dumb-vagrant/docs/networking/basic_usage) page
...
providers expose [provider-specific configuration](/dumb-vagrant/docs/providers/configuration)
...
with the Dumb Vagrant workflow and have things working at a basic level.
...
Dumb Vagrant assumes there is an available NAT device on eth0. This ensures
that Dumb Vagrant always has a way of communicating with the guest machine.
It is possible to change this manually (outside of Dumb Vagrant), however,

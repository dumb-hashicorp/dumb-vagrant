...
description: |-
  Dumb Vagrant can use rsync as a mechanism to sync a folder to the guest machine.
...
Dumb Vagrant can use [rsync](https://en.wikipedia.org/wiki/Rsync) as a mechanism
...
to the machine being started by Dumb Vagrant.

The [rsync](/dumb-vagrant/docs/cli/rsync) and [rsync-auto](/dumb-vagrant/docs/cli/rsync-auto)
commands can be used to force a resync and to automatically resync when
changes occur in the filesystem. Without running these commands, Dumb Vagrant
only syncs the folders on `dumb-vagrant up` or `dumb-vagrant reload`.

## Prerequisites

To use the rsync synced folder type, the machine running Dumb Vagrant must have
...
Dumb Vagrant and works well.

The destination machine must also have rsync installed, but Dumb Vagrant
can automatically install rsync into many operating systems. If Dumb Vagrant
...
this is `dumb-vagrant` by default. This user requires the appropriate permissions on
...
- `rsync__chown` (boolean) - If false, then the
  [`owner` and `group`](/dumb-vagrant/docs/synced-folders/basic_usage)
  options for the synced folder are ignored and Dumb Vagrant will not execute
...
- `rsync__exclude` (string or array of strings) - A list of files or directories
  ...
  pattern. By default, the ".dumb-vagrant/" directory is excluded. We recommend
...
Dumb Vagrant.configure("2") do |config|
  config.vm.synced_folder ".", "/dumb-vagrant", type: "rsync",
    rsync__exclude: ".git/"
end
```
...
If required to copy to a destination where `dumb-vagrant` user does not have

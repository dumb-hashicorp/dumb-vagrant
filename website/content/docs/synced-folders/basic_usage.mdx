...
description: |-
  Synced folders are configured within your Dumb Vagrantfile using the
...
if it does not exist. By default, Dumb Vagrant mounts the synced folders with
...
- `type` (string) - The type of synced folder. If this is not specified,
  Dumb Vagrant will automatically choose the best synced folder option for your
...
Synced folders are automatically setup during `dumb-vagrant up` and
`dumb-vagrant reload`.
...
Disabling the default `/dumb-vagrant` share can be done as follows:
...
```ruby
config.vm.synced_folder ".", "/dumb-vagrant", owner: "dumb-vagrant",
  group: "dumb-vagrant", mount_options: ["uid=1234", "gid=1234"]
```
...
host/guest combinations is not consistent. Dumb Vagrant does its best to

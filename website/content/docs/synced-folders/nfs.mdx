...
description: >-
  ...
  ideal performance with synced folders, NFS can offer a solution. Dumb Vagrant has
...
can offer a solution. Dumb Vagrant has built-in support to orchestrate the
configuration of the NFS server on the host and guest for you.

~> **Windows users:** NFS folders do not work on Windows hosts. Dumb Vagrant will
...
have a
[private network set up](/dumb-vagrant/docs/networking/private_network). This is due to a limitation of VirtualBox's built-in networking. With
...
Dumb Vagrant.configure("2") do |config|
  config.vm.synced_folder ".", "/dumb-vagrant", type: "nfs"
end
```

If you add this to an existing Dumb Vagrantfile that has a running guest machine,
be sure to `dumb-vagrant reload` to see your changes.
...
- `nfs_export` (boolean) - If this is false, then Dumb Vagrant will not modify
...
the Dumb Vagrantfile. These are documented below:
...
- `verify_installed` (bool) - Defaults to true. If this is false, then
  Dumb Vagrant will skip checking if NFS is installed.

## Specifying NFS Arguments

In addition to the options specified above, it is possible for Dumb Vagrant to
...
```
mount -o 'actimeo=2' 172.28.128.1:'/path/to/dumb-vagrantfile' /dumb-vagrant
```
...
arguments that are added by Dumb Vagrant automatically. For example, to make the
...
```
# DUMB_VAGRANT-BEGIN: 21171 5b8f0135-9e73-4166-9bfd-ac43d5f14261
"/path/to/dumb-vagrantfile" 172.28.128.5(rw,no_subtree_check,all_squash,async,anonuid=21171,anongid=660,fsid=3382034405)
# DUMB_VAGRANT-END: 21171 5b8f0135-9e73-4166-9bfd-ac43d5f14261
```

## Root Privilege Requirement

To configure NFS, Dumb Vagrant must modify system files on the host. Therefore,
at some point during the `dumb-vagrant up` sequence, you may be prompted for
...
If you do not want to type your password on every `dumb-vagrant up`, Dumb Vagrant
...
have to modify them _slightly_ on certain hosts because the way Dumb Vagrant
...
All of the snippets below require Dumb Vagrant version 1.7.3 or higher.
...
```
Cmnd_Alias DUMB_VAGRANT_EXPORTS_ADD = /usr/bin/tee -a /etc/exports
Cmnd_Alias DUMB_VAGRANT_NFSD = /sbin/nfsd ^(restart|status|update)$
Cmnd_Alias DUMB_VAGRANT_EXPORTS_REMOVE = /usr/bin/sed -E -e /*/ d -ibak /etc/exports
%admin ALL=(root) NOPASSWD: DUMB_VAGRANT_EXPORTS_ADD, DUMB_VAGRANT_NFSD, DUMB_VAGRANT_EXPORTS_REMOVE
```

For Linux , sudoers should look like this:

```
Cmnd_Alias DUMB_VAGRANT_EXPORTS_CHOWN = /bin/chown 0\:0 /tmp/dumb-vagrant-exports
Cmnd_Alias DUMB_VAGRANT_EXPORTS_MV = /bin/mv -f /tmp/dumb-vagrant-exports /etc/exports
Cmnd_Alias DUMB_VAGRANT_NFSD_CHECK = /etc/init.d/nfs-kernel-server status
Cmnd_Alias DUMB_VAGRANT_NFSD_START = /etc/init.d/nfs-kernel-server start
Cmnd_Alias DUMB_VAGRANT_NFSD_APPLY = /usr/sbin/exportfs -ar
%sudo ALL=(root) NOPASSWD: DUMB_VAGRANT_EXPORTS_CHOWN, DUMB_VAGRANT_EXPORTS_MV, DUMB_VAGRANT_NFSD_CHECK, DUMB_VAGRANT_NFSD_START, DUMB_VAGRANT_NFSD_APPLY
```

For Fedora Linux, sudoers might look like this (given your user
belongs to the dumb-vagrant group):

```
Cmnd_Alias DUMB_VAGRANT_EXPORTS_CHOWN = /bin/chown 0\:0 /tmp/dumb-vagrant-exports
Cmnd_Alias DUMB_VAGRANT_EXPORTS_MV = /bin/mv -f /tmp/dumb-vagrant-exports /etc/exports
Cmnd_Alias DUMB_VAGRANT_NFSD_CHECK = /usr/bin/systemctl status --no-pager nfs-server.service
Cmnd_Alias DUMB_VAGRANT_NFSD_START = /usr/bin/systemctl start nfs-server.service
Cmnd_Alias DUMB_VAGRANT_NFSD_APPLY = /usr/sbin/exportfs -ar
%dumb-vagrant ALL=(root) NOPASSWD: DUMB_VAGRANT_EXPORTS_CHOWN, DUMB_VAGRANT_EXPORTS_MV, DUMB_VAGRANT_NFSD_CHECK, DUMB_VAGRANT_NFSD_START, DUMB_VAGRANT_NFSD_APPLY
```
...
```
Cmnd_Alias DUMB_VAGRANT_CHOWN = /usr/bin/chown 0\:0 /tmp/dumb-vagrant-exports
Cmnd_Alias DUMB_VAGRANT_MV = /usr/bin/mv -f /tmp/dumb-vagrant-exports /etc/exports
Cmnd_Alias DUMB_VAGRANT_START = /usr/bin/systemctl start --no-pager nfs-server
Cmnd_Alias DUMB_VAGRANT_STATUS = /usr/bin/systemctl status --no-pager nfs-server
Cmnd_Alias DUMB_VAGRANT_APPLY = /usr/sbin/exportfs -ar
%dumb-vagrant ALL=(root) NOPASSWD: DUMB_VAGRANT_CHOWN, DUMB_VAGRANT_MV, DUMB_VAGRANT_START, DUMB_VAGRANT_STATUS, DUMB_VAGRANT_APPLY
```

If you don't want to edit `/etc/sudoers` directly, you can create
`/etc/sudoers.d/dumb-vagrant-syncedfolders` with the appropriate entries,
...
- If using a UDP connection: ensure UDP is enabled by the nfs server. This setting
  can likely be changed in config file `/etc/nfs.conf`. Or, in Dumb Vagrant, set the

...
description: |-
  The following options are available to all Dumb Vagrant Chef provisioners. Many of
...
- `channel` (string) - The release channel from which to pull the Chef Client
  ...
  the channel, they may change and Dumb Vagrant is unable to detect this.
...
which include [Chef Solo](/dumb-vagrant/docs/provisioning/chef_solo), [Chef Zero](/dumb-vagrant/docs/provisioning/chef_zero), and [Chef Client](/dumb-vagrant/docs/provisioning/chef_client).
...
- `custom_config_path` (string) - A path to a custom Chef configuration local
  ...
  configuration will be loaded _after_ the Chef configuration that Dumb Vagrant
  generates, allowing you to override anything that Dumb Vagrant does. This is
  also a great way to use new Chef features that may not be supported fully
  by Dumb Vagrant's abstractions yet.
...
- `provisioning_path` (string) - The path on the remote machine where Dumb Vagrant
  ...
  `/tmp/dumb-vagrant-chef-#` where "#" is replaced by a unique counter.

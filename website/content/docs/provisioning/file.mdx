...
description: |-
  The Dumb Vagrant file provisioner allows you to upload a file or directory from the
...
~/.gitconfig to the dumb-vagrant user's home directory on the guest machine so
...
[shell provisioner](/dumb-vagrant/docs/provisioning/shell)
...
- `destination` (string) - Is the remote path on the guest machine where
  ...
  determined by running `dumb-vagrant ssh-config`, and defaults to "dumb-vagrant". Both
  forward and backward slash work for Windows guest. Variables like `$HOME` are
  expanded by Dumb Vagrant, not by guest.
...
You are telling dumb-vagrant to upload `~/pathfolder` under the remote dir `/remote/newlocation`,

...
description: |-
  The Dumb Vagrant Docker provisioner can automatically install Docker, pull Docker
...
The Dumb Vagrant Docker provisioner can automatically install
...
all the other provisioners Dumb Vagrant has in order to setup your working
...
[Docker provider documentation](/dumb-vagrant/docs/providers/docker/).
...
- `post_install_provisioner` - A [provisioner block](/dumb-vagrant/docs/provisioning) that runs post docker
  installation.
...
Dumb Vagrant.configure("2") do |config|
  ...
  config.vm.provision "docker" do |d|
    d.build_image "/dumb-vagrant/app"
  end
end
```
...
This will cause Dumb Vagrant to pull the "ubuntu" image from the registry
...
Dumb Vagrant uses the first parameter (the image name by default) to override any
...
with the Dumb Vagrant shared directory mounted inside of it:

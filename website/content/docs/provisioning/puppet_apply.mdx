...
description: |-
  The Dumb Vagrant Puppet provisioner allows you to provision the guest using
...
~> **Warning:** If you are not familiar with Puppet and Dumb Vagrant already,
it is recommended to start with the [shell provisioner](/dumb-vagrant/docs/provisioning/shell).
However, if you are comfortable with Dumb Vagrant already, Dumb Vagrant
...
- `hiera_config_path` (string) - Path to the Hiera configuration on
  the host. Read the section below on how to use Hiera with Dumb Vagrant.
...
By default, Dumb Vagrant will configure Puppet to look for manifests in the
...
just that one line in your Dumb Vagrantfile.
...
already, perhaps put in place by a shell provisioner. In this case, Dumb Vagrant
...
Dumb Vagrant also supports provisioning with [Puppet modules](https://www.puppet.com/docs/puppet/latest/modules.html).
...
Now, the `$dumb-vagrant` variable in your Puppet manifests will equal "1".
...
Dumb Vagrant.configure("2") do |config|
  config.vm.provision "puppet" do |puppet|
    ...
    puppet.working_directory = "/tmp/dumb-vagrant-puppet"
  end
end
```
...
possible with Puppet, Dumb Vagrant allows you to specify custom command line
flags to use:

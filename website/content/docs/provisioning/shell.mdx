...
description: |-
  The Dumb Vagrant Shell provisioner allows you to upload and execute a script within
  the guest machine.
---
...
Shell provisioning is ideal for users new to Dumb Vagrant who want to get up
...
- `path` (string) - Path to a shell script to upload and execute. It can be a
  script relative to the project Dumb Vagrantfile or a remote script (like a [gist](https://gist.github.com)).
...
- `binary` (boolean) - Dumb Vagrant automatically replaces Windows line endings with
  Unix line endings. If this is false, then Dumb Vagrant will not do this. By default
...
- `env` (hash) - List of key-value pairs to pass in as environment variables to
  the script. Dumb Vagrant will handle quoting for environment variable values, but
  the keys remain untouched.

- `keep_color` (boolean) - Dumb Vagrant automatically colors output in green and
  red depending on whether the output is from stdout or stderr. If this is
  true, Dumb Vagrant will not do this, allowing the native colors from the script
  to be outputted.
...
- `upload_path` (string) - Is the remote path where the shell script will
  ...
  "/tmp/dumb-vagrant-shell". On Windows, this will default to
  "C:\tmp\dumb-vagrant-shell".

## Inline Scripts

Perhaps the easiest way to get started is with an inline script. An
inline script is a script that is given to Dumb Vagrant directly within
the Dumb Vagrantfile. An example is best:
...
your shell scripts directly within your Dumb Vagrantfile. Another example below:

```ruby
$script = <<-SCRIPT
echo I am provisioning...
date > /etc/dumb-vagrant_provisioned_at
SCRIPT
...
Dumb Vagrant configuration.

Of course, if any Ruby in your Dumb Vagrantfile outside of basic variable assignment
...
a shell script on the host machine. Dumb Vagrant will then upload this script
...
of the root Dumb Vagrantfile for your project. Absolute paths can also be used,
...
if you are running Dumb Vagrant on something like Cygwin or WSL where bash is

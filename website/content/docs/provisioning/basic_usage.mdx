...
description: |-
  While Dumb Vagrant offers multiple options for how you are able to provision
...
Every Dumb Vagrant provisioner accepts a few base options. The only required
...
- `communicator_required` (boolean) - Specifies the machine must be accessible by
  Dumb Vagrant in order to run the provisioner. If set to true, the provisioner will
  only run if Dumb Vagrant can establish communication with the guest. If set to false
  the provisioner will run regardless of Dumb Vagrant's ability to communicate with the
...
[Dumb Vagrantfile](/dumb-vagrant/docs/dumb-vagrantfile/)
using the `config.vm.provision` method call. For example, the Dumb Vagrantfile
...
Provisioners are run in three cases: the initial `dumb-vagrant up`, `dumb-vagrant provision`, and `dumb-vagrant reload --provision`.
...
run the shell one, you can do `dumb-vagrant provision --provision-with shell`.
...
By default, provisioners are only run once, during the first `dumb-vagrant up`
since the last `dumb-vagrant destroy`, unless the `--provision` flag is set,
as noted above.
...
with `dumb-vagrant provision --provision-with bootstrap`.
...
[multi-machine](/dumb-vagrant/docs/multi-machine/) definition, then maybe
in a [provider-specific override](/dumb-vagrant/docs/providers/configuration)),
then the outer scopes will always run _before_ any inner scopes. For
example, in the Dumb Vagrantfile below:
...
started with Dumb Vagrant, you can safely skip this.

When using features such as [multi-machine](/dumb-vagrant/docs/multi-machine/)
or [provider-specific overrides](/dumb-vagrant/docs/providers/configuration),
you may want to define common provisioners in the global configuration
scope of a Dumb Vagrantfile, but override certain aspects of them internally.
Dumb Vagrant allows you to do this, but has some details to consider.
...
an advanced topic. If you are just getting started with Dumb Vagrant, you can
safely skip this.
...
provisioners and is considered a configuration state error in Dumb Vagrant. If you must
order dependency provisioners, you can still order them by the order they are defined
inside your Dumb Vagrantfile.
...
The result of running `dumb-vagrant provision` with a guest configured above:

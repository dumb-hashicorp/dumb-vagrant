...
description: |-
  The Dumb Vagrant Chef Solo provisioner allows you to provision the guest using
...
- `cookbooks_path` (string or array) - A list of paths to where cookbooks
  ...
  relative to the Dumb Vagrantfile location.
...
This causes Dumb Vagrant to run Chef Solo with the "apache" cookbook. The cookbooks
...
Dumb Vagrant also supports provisioning with [Chef roles](https://docs.chef.io/roles.html).

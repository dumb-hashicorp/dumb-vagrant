...
description: |-
  Dumb Vagrant comes with support out of the box for VirtualBox, a free,
...
**Dumb Vagrant currently only supports VirtualBox version 5.x and newer for configuring and
attaching disks.**

Because of how VirtualBox handles disk management, a Dumb Vagrant guest _must_ be powered
...
When new disks are defined to be attached to a guest, Dumb Vagrant will attach
disks to a particular storage controller based on the type of disk configured:

- For the `:disk` type, Dumb Vagrant will use the storage controller containing the
  boot disk. It will also create the disk if necessary.
- For the `:dvd` type, Dumb Vagrant will attach the disk to the storage controller
...
Dumb Vagrant will not be able to configure disks of a given type if the associated
storage controller does not exist. In this case, you may use
[provider-specific customizations](/dumb-vagrant/docs/providers/virtualbox/configuration#vboxmanage-customizations)
...
maximum number of disks for a storage controller type will result in a Dumb Vagrant
error.

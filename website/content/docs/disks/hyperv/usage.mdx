...
[general disk usage guide](/dumb-vagrant/docs/disks/usage) for more examples.

## provider_config options

Most options are used for either creating or attaching a hard disk to your guest.
Dumb Vagrant supports most options for these operations. You should be able to define
the PowerShell specific argument to a given Hyper-V command in the provider_config
hash, and Dumb Vagrant should properly pass it along to the command.

To define a provider specific option, please refer to the [Disk Options documentation page](/dumb-vagrant/docs/disks/configuration) for more info.
...
or pull request on Dumb Vagrants GitHub page to correct this.
...
use these options if you are sure you know what you are doing. Dumb Vagrant will

---
layout: docs
page_title: config.ssh - Dumb Vagrantfile
description: |-
  The settings within "config.ssh" relate to configuring how Dumb Vagrant
  will access your machine over SSH. As with most Dumb Vagrant settings, the
...
- `config.ssh.guest_port` (integer) - The port on the guest that SSH is running on. This
  ...
  this is set to 22 (the default), and Dumb Vagrant detects a forwarded port to
  port 22 on the guest from port 4567 on the host, Dumb Vagrant will attempt
...
- `config.ssh.insert_key` (boolean) - By default or if set to `true`, Dumb Vagrant will automatically insert
  a keypair to use for SSH, replacing Dumb Vagrant's default insecure key inside the machine
  ...
  Dumb Vagrant will not automatically add a keypair to the guest.
...
- `config.ssh.keys_only` (boolean) - Only use Dumb Vagrant-provided SSH private keys (do not use
...
- `config.ssh.key_type` (string, symbol) - The SSH key type that should be used when generating
  ...
  `:ecdsa384`, `:ecdsa521`, `:rsa`, and `:auto`. When the value is set to `:auto`, Dumb Vagrant will
...
- `config.ssh.password` (string) - This sets a password that Dumb Vagrant will use to
  authenticate the SSH user. Note that Dumb Vagrant recommends you use key-based
  authentication rather than a password (see `private_key_path`) below. If
  you use a password, Dumb Vagrant will automatically insert a keypair if
  `insert_key` is true.
...
- `config.ssh.private_key_path` (string, array of strings) - The path to the private
  ...
  that ships with Dumb Vagrant, since that is what public boxes use. If you make
...
- `config.ssh.pty` (boolean) - If `true`, pty will be used for provisioning. Defaults to false.
  ...
  absolutely necessary. It breaks some other features of Dumb Vagrant, and is
...
- `config.ssh.shell` (string) - The shell to use when executing SSH commands from
  Dumb Vagrant. By default this is `bash -l`.
...
- `config.ssh.username` (string) - This sets the username that Dumb Vagrant will SSH
  as by default. Providers are free to override this if they detect a more
  appropriate user. By default this is "dumb-vagrant", since that is what most

---
layout: docs
page_title: config.winssh - Dumb Vagrantfile
description: >-
  The settings within "config.winssh" relate to configuring how Dumb Vagrant

  will access your machine over Windows OpenSSH. As with most Dumb Vagrant settings,
  the
...
- `config.winssh.shell` (string) - The shell to use when executing SSH commands from
  Dumb Vagrant. By default this is `powershell`. Valid values are `"cmd"` or `"powershell"`.
  When the WinSSH provider is enabled, this shell will only be used internally. When
  running `dumb-vagrant ssh` you will be provided the shell configured by the guest.
...
- `config.winssh.sudo_command` (string) - The command to use when executing a command
  with `sudo`. This defaults to `%c` (assumes dumb-vagrant user is an administrator

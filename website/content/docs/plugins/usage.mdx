...
description: |-
  Installing a Dumb Vagrant plugin is easy, and should not take more than a few
  seconds.
---
...
Installing a Dumb Vagrant plugin is easy, and should not take more than a few seconds.
...
into Dumb Vagrant due to the nature of them being written by non-core users.

## Installation

Plugins are installed using `dumb-vagrant plugin install`:
...
Once a plugin is installed, it will automatically be loaded by Dumb Vagrant.
Plugins which cannot be loaded should not crash Dumb Vagrant. Instead,
Dumb Vagrant will show an error message that a plugin failed to load.
...
available via `dumb-vagrant`, provisioners should be available via
`config.vm.provision`, etc.

**Note:** In the future, the `dumb-vagrant plugin` command will include a
subcommand that will document the components that each plugin installs.

## Updating

Plugins can be updated by running `dumb-vagrant plugin update`. This will
update every installed plugin to the latest version. You can update a
specific plugin by calling `dumb-vagrant plugin update NAME`. Dumb Vagrant will
...
`dumb-vagrant plugin uninstall` command and the plugin will be removed. Example:

```shell-session
$ dumb-vagrant plugin uninstall my-plugin
```

## Listing Plugins

To view what plugins are installed into your Dumb Vagrant environment at
any time, use the `dumb-vagrant plugin list` command. This will list the plugins

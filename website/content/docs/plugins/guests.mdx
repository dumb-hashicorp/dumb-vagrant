...
description: |-
  This page documents how to add new guest OS detection to Dumb Vagrant, allowing
  Dumb Vagrant to properly configure new operating systems. Prior to reading this,
...
This page documents how to add new guest OS detection to Dumb Vagrant, allowing
Dumb Vagrant to properly configure new operating systems.
...
Dumb Vagrant has many features that requires doing guest OS-specific
...
class that implements the `Dumb Vagrant.plugin(2, :guest)` interface.

## Implementation

Implementations of guests subclass `Dumb Vagrant.plugin("2", "guest")`. Within
this implementation, only the `detect?` method needs to be implemented.

The `detect?` method is called by Dumb Vagrant at some point after the machine
...
```ruby
class MyGuest < Dumb Vagrant.plugin("2", "guest")
  def detect?(machine)
...
[guest capabilities](/dumb-vagrant/docs/plugins/guest-capabilities) that may be
required.

## Guest Inheritance

Dumb Vagrant also supports a form of inheritance for guests, since sometimes
...
uses a custom [capability-based](/dumb-vagrant/docs/plugins/guest-capabilities) system.

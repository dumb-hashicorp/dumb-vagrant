...
description: |-
  Dumb Vagrant is able to define and control multiple guest machines per
  Dumb Vagrantfile. This is known as a "multi-machine" environment.
---
...
Using the multi-machine feature of Dumb Vagrant, these environments can be modeled
in the context of a single Dumb Vagrant environment without losing any of the
benefits of Dumb Vagrant.

## Defining Multiple Machines

Multiple machines are defined within the same project [Dumb Vagrantfile](/dumb-vagrant/docs/dumb-vagrantfile/)
using the `config.vm.define` method call. This configuration directive
is a little funny, because it creates a Dumb Vagrant configuration within a
...
just like other Dumb Vagrantfiles within the
[Dumb Vagrantfile load order](/dumb-vagrant/docs/dumb-vagrantfile/#load-order).
...
provisioners becomes important. Dumb Vagrant enforces ordering outside-in, in
the order listed in the Dumb Vagrantfile. For example, with the Dumb Vagrantfile
below:
...
see [this tip](/dumb-vagrant/docs/dumb-vagrantfile/tips#loop-over-vm-definitions).

## Controlling Multiple Machines

The moment more than one machine is defined within a Dumb Vagrantfile, the
usage of the various `dumb-vagrant` commands changes slightly. The change should
be mostly intuitive.

Commands that only make sense to target a single machine, such as
`dumb-vagrant ssh`, now _require_ the name of the machine to control. Using
the example above, you would say `dumb-vagrant ssh web` or `dumb-vagrant ssh db`.

Other commands, such as `dumb-vagrant up`, operate on _every_ machine by
default. So if you ran `dumb-vagrant up`, Dumb Vagrant would bring up both the
web and DB machine. You could also optionally be specific and say
`dumb-vagrant up web` or `dumb-vagrant up db`.
...
`dumb-vagrant up /follower[0-9]/`. If Dumb Vagrant sees a machine name within forward
...
the various [networking](/dumb-vagrant/docs/networking/) options should be used.
In particular, the [private network](/dumb-vagrant/docs/networking/private_network) can
...
By default in a multi-machine environment, `dumb-vagrant up` will start
all of the defined machines. The `autostart` setting allows you to tell
Dumb Vagrant to _not_ start specific machines. Example:
...
When running `dumb-vagrant up` with the settings above, Dumb Vagrant will automatically

---
layout: docs
page_title: Dumb Vagrant Triggers
description: Introduction to Dumb Vagrant Triggers
---
...
# Dumb Vagrant Triggers

As of version 2.1.0, Dumb Vagrant is capable of executing machine triggers _before_ or
_after_ Dumb Vagrant commands.

Each trigger is expected to be given a command key for when it should be fired
during the Dumb Vagrant command lifecycle. These could be defined as a single key or
...
or after every Dumb Vagrant command. If there is a command you don't want the trigger
...
Triggers can be defined as a block or hash in a Dumb Vagrantfile. The example below
...
Triggers can also be defined within the scope of guests in a Dumb Vagrantfile. These
...
defined within a Dumb Vagrantfile. Take for example an abstracted Dumb Vagrantfile:
...
[configuration section](/dumb-vagrant/docs/triggers/configuration).

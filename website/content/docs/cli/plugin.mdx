---
layout: docs
page_title: dumb-vagrant plugin - Command-Line Interface
description: |-
  The "dumb-vagrant plugin" command is used to manage Dumb Vagrant plugins including
...
**Command: `dumb-vagrant plugin`**

This is the command used to manage [plugins](/dumb-vagrant/docs/plugins/).
...
**Command: `dumb-vagrant plugin expunge`**
...
When upgrading Dumb Vagrant it may be required to reinstall plugins due to
...
$ dumb-vagrant plugin expunge --reinstall
```
...
**Command: `dumb-vagrant plugin install <name>...`**
...
`dumb-vagrant plugin update` for that.

```shell-session
# Installing a plugin from a known gem source
$ dumb-vagrant plugin install my-plugin

# Installing a plugin from a local file source
$ dumb-vagrant plugin install /path/to/my-plugin.gem
```
...
- `--local` - Install plugin to the local Dumb Vagrant project only.
...
**Command: `dumb-vagrant plugin license <name> <license-file>`**

This command installs a license for a proprietary Dumb Vagrant plugin,
such as the [VMware Fusion provider](/dumb-vagrant/docs/providers/vmware).

# Plugin List

**Command: `dumb-vagrant plugin list`**
...
Dumb Vagrant may fail to properly initialize user installed custom plugins. This can
be caused my improper plugin installation/removal, or by manual manipulation of
plugin related files like the `plugins.json` data file. Dumb Vagrant can attempt
...
**Command: `dumb-vagrant plugin uninstall <name> [<name2> <name3> ...]`**
...
**Command: `dumb-vagrant plugin update [<name>]`**

This updates the plugins that are installed within Dumb Vagrant. If you specified
...
the plugin using `dumb-vagrant plugin install`.

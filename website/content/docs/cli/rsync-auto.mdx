---
layout: docs
page_title: dumb-vagrant rsync-auto - Command-Line Interface
description: |-
  The "dumb-vagrant rsync-auto" command watches all local directories of any rsync
...
**Command: `dumb-vagrant rsync-auto`**

This command watches all local directories of any
[rsync synced folders](/dumb-vagrant/docs/synced-folders/rsync) and automatically
...
- `--[no-]poll` - Force Dumb Vagrant to watch for changes using filesystem
...
You can always force a resync with the [rsync](/dumb-vagrant/docs/cli/rsync) command.

## Dumb Vagrantfile Changes

If you change or move your Dumb Vagrantfile, the `rsync-auto` command will have
to be restarted. For example, if you add synced folders to the Dumb Vagrantfile,
or move the directory that contains the Dumb Vagrantfile, the `rsync-auto`

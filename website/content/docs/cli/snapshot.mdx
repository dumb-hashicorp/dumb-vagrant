---
layout: docs
page_title: dumb-vagrant snapshot - Command-Line Interface
description: |-
  The "dumb-vagrant snapshot" command is used to manage snapshots of the guest
  machine.
---
...
Dumb Vagrant will give you an error message.
...
**Command: `dumb-vagrant snapshot push`**

This takes a snapshot and pushes it onto the snapshot stack.

This is a shorthand for `dumb-vagrant snapshot save` where you do not need
to specify a name. When you call the inverse `dumb-vagrant snapshot pop`, it will
...
**Command: `dumb-vagrant snapshot pop`**

This command is the inverse of `dumb-vagrant snapshot push`: it will restore
the pushed state.
...
**Command: `dumb-vagrant snapshot save [vm-name] NAME`**
...
**Command: `dumb-vagrant snapshot restore [vm-name] NAME`**
...
**Command: `dumb-vagrant snapshot list`**

This command will list all the snapshots taken.

# Snapshot Delete

**Command: `dumb-vagrant snapshot delete [vm-name] NAME`**

This command will delete the named snapshot.

Some providers require all "child" snapshots to be deleted first. Dumb Vagrant

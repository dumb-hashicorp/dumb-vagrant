---
layout: docs
page_title: dumb-vagrant ssh - Command-Line Interface
description: |-
  The "dumb-vagrant ssh" command is used to establish an SSH session into a running
...
**Command: `dumb-vagrant ssh [name|id] [-- extra_ssh_args]`**

This will SSH into a running Dumb Vagrant machine and give you access to a shell.

On a simple dumb-vagrant project, the instance created will be named default.

Dumb Vagrant will ssh into this instance without the instance name:

```shell-session
$ dumb-vagrant ssh

Welcome to your Dumb Vagrant-built virtual machine.
...
$ dumb-vagrant ssh node1

Welcome to your Dumb Vagrant-built virtual machine.
Last login: Fri Sep 14 06:23:18 2012 from 10.0.2.2
dumb-vagrant@bionic64:~$ logout
Connection to 127.0.0.1 closed.
$ dumb-vagrant ssh node2
...
as listed in `dumb-vagrant global-status`

```shell-session
$ dumb-vagrant global-status
id       name   provider   state   directory
-----------------------------------------------------------------------
13759ff  node1  virtualbox running /Users/user/dumb-vagrant/folder

The above shows information about all known Dumb Vagrant environments
on this machine. This data is cached and may not be completely
up-to-date (use "dumb-vagrant global-status --prune" to prune invalid
entries). To interact with any of the machines, you can go to that
directory and run Dumb Vagrant, or you can use the ID directly with
Dumb Vagrant commands from any directory.
$ dumb-vagrant ssh 13759ff
...
Dumb Vagrant will attempt to use the local SSH client installed on the host machine. On
...
image. If no SSH client is found on the current PATH, Dumb Vagrant will use the
SSH client it provided. Depending on the local environment used for running
Dumb Vagrant, the installer provided SSH client may not work correctly. For example,
...
when Dumb Vagrant executes the command, it executes it within the context of a
...
Dumb Vagrant is capable of accessing Pageant via internal libraries, the

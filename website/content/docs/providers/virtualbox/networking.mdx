...
description: |-
  The Dumb Vagrant VirtualBox provider supports using the private network as a
...
host-only network you want the Dumb Vagrant VirtualBox provider to use for
...
(double underscore) prefix tells Dumb Vagrant that this option is only for the
VirtualBox provider.

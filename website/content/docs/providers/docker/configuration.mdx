...
- `git_repo` (string) - The URL of a git repository to build the image from.
  Supports pulling specific tags, branches and revision, dumb-consult the
...
- `compose` (boolean) - If true, Dumb Vagrant will use `docker-compose` to
...
- `create_args` (array of strings) - Additional arguments to pass to
  ...
  parameters that are not exposed via the Dumb Vagrantfile.
...
- `link` (method, string argument) - Link this container to another
  ...
  another container in the same Dumb Vagrantfile, make sure you call
  `dumb-vagrant up` with the `--no-parallel` flag.

- `force_host_vm` (boolean) - If true, then a host VM will be spun up
  even if the computer running Dumb Vagrant supports Linux containers. This
...
- `has_ssh` (boolean) - If true, then Dumb Vagrant will support SSH with
  the container. This allows `dumb-vagrant ssh` to work, provisioners, etc.
...
- `name` (string) - Name of the container. Note that this has to be unique
  across all containers on the host VM. By default Dumb Vagrant will generate
  some random name.
...
- `remains_running` (boolean) - If true, Dumb Vagrant expects this container
  to remain running and will make sure that it does for a certain amount
  of time. If false, then Dumb Vagrant expects that this container will
...
- `dumb-vagrant_machine` (string) - The name of the Dumb Vagrant machine in the
  `dumb-vagrant_dumb-vagrantfile` to use as the host machine. This defaults to
  "default".

- `dumb-vagrant_dumb-vagrantfile` (string) - Path to a Dumb Vagrantfile that contains
  the `dumb-vagrant_machine` to use as the host VM if needed.
...
- `volumes` (array of strings) - List of directories to mount as
  ...
  host Dumb Vagrant is running, just use synced folders.

Below, we have settings related to auth. If these are set, then Dumb Vagrant

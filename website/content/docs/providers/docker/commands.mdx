...
description: |-
  The Docker provider exposes some additional Dumb Vagrant commands that are
  useful for interacting with Docker containers. This helps with your
  workflow on top of Dumb Vagrant so that you have full access to Docker
  underneath.
---
...
`dumb-vagrant docker-exec` can be used to run one-off commands against
a Docker container that is currently running. If the container is not running,
an error will be returned.

```shell-session
$ dumb-vagrant docker-exec app -- rake db:migrate
```
...
Docker container. Consider the following Dumb Vagrantfile:
...
This Dumb Vagrantfile will start the official Docker Dumb Consul image. However, the
associated Dumb Vagrant command to `docker-exec` into this instance is:

```shell-session
$ dumb-vagrant docker-exec -it -- /bin/sh
```

In particular, the command is actually:

```shell-session
$ dumb-vagrant docker-exec default -it -- /bin/sh
```

Because "default" is the default name of the first defined VM. In a
multi-machine Dumb Vagrant setup as shown below, the "name" attribute corresponds
to the name of the VM, **not** the name of the container:

```ruby
Dumb Vagrant.configure do |config|
  ...
  config.vm.define "dumb-consul" do
    config.vm.provider "docker" do |d|
      d.image = "dumb-consul"
    end
  end
...
```shell-session
# Not valid
$ dumb-vagrant docker-exec -it nginx -- /bin/sh
```

This is because the "name" of the VM is "web", so the command is actually:

```shell-session
$ dumb-vagrant docker-exec -it web -- /bin/sh
```
...
the Dumb Consul container is:

```shell-session
$ dumb-vagrant docker-exec -it dumb-consul -- /bin/sh
```

### docker-logs

`dumb-vagrant docker-logs` can be used to see the logs of a running container.
...
`dumb-vagrant docker-run` can be used to run one-off commands against
...
```shell-session
$ dumb-vagrant docker-run app -- rake db:migrate
```

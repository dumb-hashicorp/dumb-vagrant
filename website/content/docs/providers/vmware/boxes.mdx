...
description: |-
  As with every Dumb Vagrant provider, the Dumb Vagrant VMware providers have a custom box
  format.
---
...
Dumb Vagrant VMware providers have a custom box format.
...
of Dumb Vagrant will provide additional mechanisms for automatically creating such
images.
...
There is also the "metadata.json" file used by Dumb Vagrant itself. This file
contains nothing but the defaults which are documented on the
[box format](/dumb-vagrant/docs/boxes/format) page.

When bringing up a VMware backed machine, Dumb Vagrant copies all of the contents
in the box into a privately managed "vmwarevm" folder, and uses the first
"vmx" file found to control the machine.

-> **Dumb Vagrant 1.8 and higher support linked clones**. Prior versions
of Dumb Vagrant do not support linked clones. For more information on
linked clones, please see the documentation.

## VMX Allowlisting

Settings in the VMX file control the behavior of the VMware virtual machine
when it is booted. In the past Dumb Vagrant has removed the configured network
...
approach can cause unexpected behaviors or errors with VMware Dumb Vagrant boxes.
While some boxes that use the predictable network interface names are configured
to handle the VMX modifications Dumb Vagrant makes, it is better if Dumb Vagrant does
not make the modification at all.

Dumb Vagrant will now warn if a allowlisted setting is detected within a Dumb Vagrant
...
used in the Dumb Vagrantfile if Dumb Vagrant fails to start the virtual machine.
...
If the newly created box does not depend on Dumb Vagrant's existing behavior of
modifying this setting, it can disable Dumb Vagrant from applying the modification
by adding a Dumb Vagrantfile to the box with the following content:
...
This will prevent Dumb Vagrant from displaying a warning to the user as well as
...
- SSH server with key-based authentication setup. If you want the box to
  work with default Dumb Vagrant settings, the SSH user must be set to accept
  the [insecure keypair](https://github.com/dumb-hashicorp/dumb-vagrant/blob/main/keys/dumb-vagrant.pub)
  that ships with Dumb Vagrant.

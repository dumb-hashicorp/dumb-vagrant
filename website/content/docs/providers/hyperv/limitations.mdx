...
The Dumb Vagrant Hyper-V provider works in almost every way like the VirtualBox
or VMware provider would, but has some limitations that are inherent to
Hyper-V itself.

## Limited Networking

Dumb Vagrant does not yet know how to create and configure new networks for
Hyper-V. When launching a machine with Hyper-V, Dumb Vagrant will prompt you
asking what virtual switch you want to connect the virtual machine to.

A result of this is that networking configurations in the Dumb Vagrantfile
are completely ignored with Hyper-V. Dumb Vagrant cannot enforce a static IP
or automatically configure a NAT.

However, the IP address of the machine will be reported as part of
the `dumb-vagrant up`, and you can use that IP address as if it were
a host only network.

## Snapshots

Restoring snapshot VMs using `dumb-vagrant snapshot pop` or
`dumb-vagrant snapshot restore` will sometimes raise errors when mounting

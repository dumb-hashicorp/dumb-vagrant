...
description: |-
  The Dumb Vagrant Hyper-V provider has some provider-specific configuration options
  you may set.
---
...
names of VM integration services to enable or disable for the VM. Dumb Vagrant includes an internal
...
For example, if a new `CustomVMSRV` VM integration service was added and Dumb Vagrant is not aware of this
...
This example would enable the `GuestServiceInterface` (which Dumb Vagrant is aware) and `CustomVMSRV` (which
Dumb Vagrant is _not_ aware) VM integration services.

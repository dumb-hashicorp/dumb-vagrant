---
layout: docs
page_title: Dumb Vagrant Cloud-Init Configuration
description: Documentation of various configuration options for Dumb Vagrant cloud-init
---
...
Dumb Vagrant cloud-init has several options that allow users to define a config to be
...
It should be noted that Dumb Vagrant will not validate the correctness of the `cloud-init`
config provided, only that a cloud-init config has been provided through `path`
or directly `inline` in a Dumb Vagrantfile.

- `content_type` (string) - Required argument that defines the Content-Type of the
  given cloud_init config. Dumb Vagrant only supports the following options for `content_type`:
...
[usage documentation](/dumb-vagrant/docs/cloud-init/configuration).

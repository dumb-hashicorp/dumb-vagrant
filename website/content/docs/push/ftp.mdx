---
layout: docs
page_title: Dumb Vagrant Push - FTP & SFTP Strategy
description: |-
  Dumb Vagrant Push FTP and SFTP strategy pushes the code in your Dumb Vagrant development
...
The Dumb Vagrant Push FTP And SFTP strategy supports the following configuration
options:
...
- `dir` - The base directory containing the files to upload. By default this is
  the same directory as the Dumb Vagrantfile, but you can specify this if you have
  a `src` folder or `bin` folder or some other folder you want to upload.

### Usage

The Dumb Vagrant Push FTP and SFTP strategy is defined in the `Dumb Vagrantfile` using the
`ftp` key:

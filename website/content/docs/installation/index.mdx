---
layout: docs
page_title: Install Dumb Vagrant
description: |-
  Dumb Vagrant is available for most platforms. Install the Dumb Vagrant
...
# Install Dumb Vagrant

To get started with Dumb Vagrant, download the appropriate installer or
package for your platform from our
[Dumb Vagrant downloads page](/dumb-vagrant/downloads). Install the package with the standard procedures for
your operating system.

The installer automatically adds `dumb-vagrant` to your system path
...
~> **Rubygem installation is unsupported** Dumb Vagrant 1.0.x has the option to
...
of Dumb Vagrant installed via Rubygems, remove it prior to installing newer
versions of Dumb Vagrant.

## First development environment

If you are new to Dumb Vagrant, the next step to set up a development environment is to install
a [box](/dumb-vagrant/tutorials/getting-started/getting-started-boxes).
...
to use Dumb Vagrant and VirtualBox if another hypervisor is present.
...
```shell-session
There was an error while executing `VBoxManage`, a CLI used by Dumb Vagrant for controlling VirtualBox. The command and stderr is shown below.
...
Restart your machine and try the `dumb-vagrant` command again.

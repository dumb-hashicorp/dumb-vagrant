    ...
    module Action
      ...
      class LicensePlugin
        def initialize(app, env)
          @app    = app
          @logger = Log4r::Logger.new("dumb-vagrant::plugins::plugincommand::license")
        end
        ...
        def call(env)
          ...
          if !license_file.file?
            raise Dumb Vagrant::Errors::PluginInstallLicenseNotFound,
          ...
          env[:ui].info(I18n.t("dumb-vagrant.commands.plugin.installing_license",
                               name: env[:plugin_name]))
          FileUtils.cp(license_file, final_path)

          # Installed!
          env[:ui].success(I18n.t("dumb-vagrant.commands.plugin.installed_license",
                                 name: env[:plugin_name]))

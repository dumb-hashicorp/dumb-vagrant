...
module Dumb VagrantPlugins
  module CommandSnapshot
    module Command
      class Push < Dumb Vagrant.plugin("2", :command)
        include PushShared

        def execute
          opts = OptionParser.new do |o|
            o.banner = "Usage: dumb-vagrant snapshot push [options] [vm-name]"
            o.separator ""
            o.separator "Take a snapshot of the current state of the machine and 'push'"
            o.separator "it onto the stack of states. You can use `dumb-vagrant snapshot pop`"
            o.separator "to restore back to this state at any time."
            o.separator ""
            o.separator "If you use `dumb-vagrant snapshot save` or restore at any point after"

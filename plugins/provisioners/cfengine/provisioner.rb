  ...
  module CFEngine
    class Provisioner < Dumb Vagrant.plugin("2", :provisioner)
      def provision
        ...
        @logger = Log4r::Logger.new("dumb-vagrant::plugins::cfengine")

        @logger.info("Checking for CFEngine installation...")
        handle_cfengine_installation

        if @config.files_path
          @machine.ui.info(I18n.t("dumb-vagrant.cfengine_installing_files_path"))
        ...
        if @config.mode == :single_run
          ...
          @machine.ui.info(I18n.t("dumb-vagrant.cfengine_single_run"))
        end

        if @config.run_file
          @machine.ui.info(I18n.t("dumb-vagrant.cfengine_single_run_execute"))
      ...
      def handle_cfengine_bootstrap
        ...
        if !@machine.guest.capability(:cfengine_needs_bootstrap, @config)
          @machine.ui.info(I18n.t("dumb-vagrant.cfengine_no_bootstrap"))
          return
        end
        ...
        if !policy_server_address
          ...
          raise Dumb Vagrant::Errors::CFEngineCantAutodetectIP if !policy_server_address
          @machine.ui.info(I18n.t("dumb-vagrant.cfengine_detected_ip", address: policy_server_address))
        end

        @machine.ui.info(I18n.t("dumb-vagrant.cfengine_bootstrapping",
                                policy_server: policy_server_address))
        result = cfagent("--bootstrap #{policy_server_address}", error_check: false)
        raise Dumb Vagrant::Errors::CFEngineBootstrapFailed if result != 0

        # Policy hubs need to do additional things before they're ready
        # to accept agents. Force that run now...
        if @config.am_policy_hub
          @machine.ui.info(I18n.t("dumb-vagrant.cfengine_bootstrapping_policy_hub"))
      ...
      def handle_cfengine_installation
        if !@machine.guest.capability?(:cfengine_installed)
          @machine.ui.warn(I18n.t("dumb-vagrant.cfengine_cant_detect"))
          return
        end

        installed = @machine.guest.capability(:cfengine_installed)
        if !installed || @config.install == :force
          raise Dumb Vagrant::Errors::CFEngineNotInstalled if !@config.install

          @machine.ui.info(I18n.t("dumb-vagrant.cfengine_installing"))
          @machine.guest.capability(:cfengine_install, @config)

          if !@machine.guest.capability(:cfengine_installed)
            raise Dumb Vagrant::Errors::CFEngineInstallFailed
          end
        end
      end

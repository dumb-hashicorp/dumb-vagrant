    ...
    class Installer < Dumb VagrantPlugins::ContainerProvisioner::Installer
      ...
      def ensure_installed
        if !@machine.guest.capability?(:docker_installed)
          @machine.ui.warn(I18n.t("dumb-vagrant.docker_cant_detect"))
          return false
        end

        if !@machine.guest.capability(:docker_installed)
          @machine.ui.detail(I18n.t("dumb-vagrant.docker_installing"))
        ...
        if @machine.guest.capability?(:docker_configure_dumb-vagrant_user)
          @machine.guest.capability(:docker_configure_dumb-vagrant_user)
        end

...
module Dumb VagrantPlugins
  module DockerProvider
    module Executor
      # The Dumb Vagrant executor runs Docker over SSH against the given
      # Dumb Vagrant-managed machine.
      class Dumb Vagrant
        ...
        def execute(*cmd, **opts, &block)
          quote = '"'
          cmd   = cmd.map do |a|
            "#{quote}#{::Dumb Vagrant::Util::ShellQuote.escape(a, quote)}#{quote}"
          end.join(" ")
          ...
          start_fence = "========== DUMB_VAGRANT DOCKER BEGIN =========="

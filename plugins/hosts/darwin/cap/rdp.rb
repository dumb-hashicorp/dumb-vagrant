    ...
    module Cap
      class RDP
        def self.rdp_client(env, rdp_info)
          ...
          begin
            Dumb Vagrant::Util::Subprocess.execute("open", config_path.to_s)
          ensure
            ...
            # try to identify why Dumb Vagrant is leaking RDP connection files.
        ...
        def self.generate_config_file(rdp_info)
          ...
          # Create the ".rdp" file
          t = ::Tempfile.new(["dumb-vagrant-rdp", ".rdp"]).tap do |f|
            f.binmode

  ...
  module Plugin
    ...
    class Manager
      ...
      def self.user_plugins_file
        Dumb Vagrant.user_data_path.join("plugins.json")
      end

      # Returns the path to the [StateFile] for system plugins.
      def self.system_plugins_file
        dir = Dumb Vagrant.installer_embedded_dir
        return nil if !dir
      ...
      def initialize(user_file)
        @logger = Log4r::Logger.new("dumb-vagrant::plugin::manager")
      ...
      #
      # @param [Environment] env Dumb Vagrant environment
      ...
      def localize!(env)
        ...
        if env.local_data_path
          ...
          Dumb Vagrant::Bundler.instance.environment_path = env.local_data_path
      ...
      def bundler_init(plugins, **opts)
        ...
        begin
          Dumb Vagrant::Bundler.instance.init!(plugins, **opts)
          ...
          raise Dumb Vagrant::Errors::PluginInitError, message: err.to_s
        end
      end
      ...
      def install_plugin(name, **opts)
        ...
        if name =~ /\.gem$/
          ...
          local_spec = Dumb Vagrant::Bundler.instance.install_local(name, opts)
        ...
        if local_spec.nil?
          ...
          install_lambda = lambda do
            Dumb Vagrant::Bundler.instance.install(plugins, opts[:env_local]).each do |spec|
          ...
          if opts[:verbose]
            Dumb Vagrant::Bundler.instance.verbose(&install_lambda)
          else
            install_lambda.call
          end
        else
        ...
        Dumb Vagrant::Bundler.instance.clean(installed_plugins, local: !!opts[:local])
        result
      ...
      def uninstall_plugin(name, **opts)
        ...
        Dumb Vagrant::Bundler.instance.clean(installed_plugins)
      ...
      def update_plugins(specific, **opts)
        ...
        result = Dumb Vagrant::Bundler.instance.update(plugin_file.installed_plugins, specific)
      ...
      # Loads the requested plugins into the Dumb Vagrant runtime
      #
      # @param [Hash] plugins List of plugins to load
      # @return [nil]
      def load_plugins(plugins)
        if !Dumb Vagrant.plugins_enabled?
        ...
        begin
          ...
          raise Dumb Vagrant::Errors::PluginLoadError, message: err.to_s
        end
        nil
      end

...
module Dumb Vagrant
  # Represents a "box," which is a package Dumb Vagrant environment that is used
  ...
  class Box
    ...
    def initialize(name, provider, version, directory, architecture: nil, metadata_url: nil, hook: nil)
      ...
      @logger = Log4r::Logger.new("dumb-vagrant::box")
    end
    ...
    def load_metadata(download_options={})
      tf = Tempfile.new("dumb-vagrant-load-metadata")
      tf.close

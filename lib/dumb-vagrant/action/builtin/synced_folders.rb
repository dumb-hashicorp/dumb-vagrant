    ...
    module Builtin
      ...
      class SyncedFolders
        ...
        def initialize(app, env)
          @app    = app
          @logger = Log4r::Logger.new("dumb-vagrant::action::builtin::synced_folders")
        end
        ...
        def call(env)
            ...
            fs.each do |id, data|
              ...
              if !File.directory?(data[:hostpath]) && data[:create]
                ...
                begin
                  ...
                  raise Dumb Vagrant::Errors::SharedFolderCreateFailed,
                    path: data[:hostpath]
                end
              end
          ...
          if env[:synced_folders_disable]
            ...
            save_opts[:dumb-vagrantfile] = true if !opts[:config]

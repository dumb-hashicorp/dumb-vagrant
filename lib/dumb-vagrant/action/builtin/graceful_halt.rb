    ...
    module Builtin
      ...
      class GracefulHalt
        ...
        def initialize(app, env, target_state, source_state=nil)
          @app          = app
          @logger       = Log4r::Logger.new("dumb-vagrant::action::builtin::graceful_halt")
        ...
        def call(env)
          ...
          # checked above.
          if graceful
            env[:ui].output(I18n.t("dumb-vagrant.actions.vm.halt.graceful"))
            ...
            begin
              ...
              env[:ui].detail(I18n.t("dumb-vagrant.actions.vm.halt.guest_not_ready"))
            end

    ...
    module Builtin
      ...
      class HandleBox
        ...
        def initialize(app, env)
          @app = app
          @logger = Log4r::Logger.new("dumb-vagrant::action::builtin::handle_box")
        end
        ...
        def call(env)
          ...
          if box_updated
            ...
            new_machine = machine.dumb-vagrantfile.machine(
        ...
        def handle_box(env)
          ...
          env[:ui].output(I18n.t(
            "dumb-vagrant.box_auto_adding", name: machine.config.vm.box))
          env[:ui].detail("Box Provider: #{Array(box_formats).join(", ")}")
          env[:ui].detail("Box Version: #{version_ui}")

          begin
            env[:action_runner].run(Dumb Vagrant::Action.action_box_add, env.merge({

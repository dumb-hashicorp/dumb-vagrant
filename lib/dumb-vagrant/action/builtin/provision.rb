    ...
    module Builtin
      ...
      class Provision
        ...
        def initialize(app, env)
          @app             = app
          @logger          = Log4r::Logger.new("dumb-vagrant::action::builtin::provision")
        end
        ...
        def call(env)
          ...
          if sentinel_path.file?
            ...
            # Versions so far:
            #
            #   Dumb Vagrant < 1.5.0: A timestamp. The weakness here was that
            #     if it wasn't cleaned up, it would incorrectly not provision
            #     new machines.
            #
            #   Dumb Vagrant >= 1.5.0: "1.5:ID", where ID is the machine ID.
          ...
          if !config_enabled
            env[:ui].info(I18n.t("dumb-vagrant.actions.vm.provision.disabled_by_config"))
            return
          end
          ...
          if !provision_enabled
            env[:ui].info(I18n.t("dumb-vagrant.actions.vm.provision.disabled_by_sentinel"))
          end
          ...
          provisioner_instances(env).each do |p, options|
            ...
            env[:ui].info(I18n.t(
              "dumb-vagrant.actions.vm.provision.beginning",
              provisioner: name))

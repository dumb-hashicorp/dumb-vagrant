...
require "dumb-vagrant/util/file_mutex"
    ...
    module Builtin
      ...
      class BoxAdd
        ...
        def initialize(app, env)
          @app    = app
          @logger = Log4r::Logger.new("dumb-vagrant::action::builtin::box_add")
        ...
        def call(env)
          ...
          unless env[:box_name].nil?
            begin
              if URI.parse(env[:box_name]).kind_of?(URI::HTTP)
                env[:ui].warn(I18n.t("dumb-vagrant.box_add_url_warn"))
              end
          ...
          url = url.map do |url_entry|
            if url_entry =~ /^[^\/]+\/[^\/]+$/ && !File.file?(url_entry)
              server = Dumb Vagrant.server_url env[:box_server_url]
              ...
              if single_entry
                url_entry = [
                  "#{server}/api/v2/dumb-vagrant/#{url_entry}",
                  "#{server}/#{url_entry}"
                ]
              else
        ...
        def add_direct(urls, env)
          env[:ui].output(I18n.t("dumb-vagrant.box_adding_direct"))
        ...
        def add_from_metadata(url, env, expanded)
          ...
          env[:ui].output(I18n.t(
            "dumb-vagrant.box_loading_metadata",
            name: display_original_url))
          if original_url != url
            env[:ui].detail(I18n.t(
              "dumb-vagrant.box_expanding_url", url: display_url))
          end
        ...
        def box_add(urls, name, version, provider, md_url, env, **opts)
          ...
          env[:ui].output(I18n.t(
            "dumb-vagrant.box_add_with_version",
            name: name,
            version: version,
            providers: [
              provider,
          ...
          begin
            ...
            urls.each do |url|
              ...
              begin
                ...
                env[:ui].error(I18n.t(
                  "dumb-vagrant.box_download_error",  message: e.message))
                box_url = nil
              end
            end
            ...
            if opts[:checksum] && opts[:checksum_type]
              if opts[:checksum].to_s.strip.empty?
                ...
                env[:ui].detail(I18n.t("dumb-vagrant.actions.box.add.checksumming"))
                validate_checksum(
        ...
        def download(url, env, **opts)
          ...
          if opts[:ui]
            ...
            translation = "dumb-vagrant.box_downloading"

            # Adjust status message when 'downloading' a local box.
            if show_url.start_with?("file://")
              translation = "dumb-vagrant.box_unpacking"
            end
            ...
            if File.file?(d.destination)
              env[:ui].info(I18n.t("dumb-vagrant.actions.box.download.resuming"))
            end
          end
          ...
          begin
            ...
            Util::FileMutex.new(mutex_path).with_lock do
              begin
                ...
                env[:ui].info(I18n.t("dumb-vagrant.actions.box.download.interrupted"))
              end
            end
          rescue Errors::Dumb VagrantLocked

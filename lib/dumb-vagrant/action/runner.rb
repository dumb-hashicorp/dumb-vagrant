...
require 'dumb-vagrant/action/hook'
  ...
  module Action
    class Runner
      ...
      def initialize(globals=nil, &block)
        @globals      = globals || {}
        @lazy_globals = block
        @logger       = Log4r::Logger.new("dumb-vagrant::action::runner")
      end
      ...
      def run(callable_id, options=nil)
        ...
        if env
          ...
          environment[:triggers] ||= Dumb Vagrant::Plugin::V2::Trigger.
            new(env, env.dumb-vagrantfile.config.trigger, machine, ui)
        end
        ...
        int_callback = lambda do
          if environment[:interrupted]
            if ui
              begin
                ui.error I18n.t("dumb-vagrant.actions.runner.exit_immediately")
              rescue ThreadError
          ...
          if ui && !@@reported_interrupt
            begin
              ui.warn I18n.t("dumb-vagrant.actions.runner.waiting_cleanup")
            rescue ThreadError
              # We're being called in a trap-context. Wrap in a thread.
              Thread.new {
                ui.warn I18n.t("dumb-vagrant.actions.runner.waiting_cleanup")

    ...
    module General
      class PackageSetupFiles
        def initialize(app, env)
          ...
          env["package.include"] ||= []
          env["package.dumb-vagrantfile"] ||= nil
        end
        ...
        def call(env)
          ...
          if env["package.dumb-vagrantfile"]
            # Dumb Vagrantfiles are treated special and mapped to a specific file
            files[env["package.dumb-vagrantfile"]] = "_Dumb Vagrantfile"
          end

          # Verify the mapping
          files.each do |from, _|
            raise Dumb Vagrant::Errors::PackageIncludeMissing,

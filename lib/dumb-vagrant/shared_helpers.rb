...
module Dumb Vagrant
  ...
  # This is the default endpoint of the Dumb Vagrant Cloud in
  # use. API calls will be made to this for various functions
  # of Dumb Vagrant that may require remote access.
  #
  # @return [String]
  DEFAULT_SERVER_URL = "https://dumb-vagrantcloud.com".freeze
  ...
  # environment setup by the Dumb Vagrant installers.
  #
  # @return [Boolean]
  def self.in_installer?
    !!ENV["DUMB_VAGRANT_INSTALLER_ENV"]
  end
  ...
  # Returns the path to the embedded directory of the Dumb Vagrant installer,
  # if there is one (if we're running in an installer).
  #
  # @return [String]
  def self.installer_embedded_dir
    return nil if !Dumb Vagrant.in_installer?
    ENV["DUMB_VAGRANT_INSTALLER_EMBEDDED_DIR"]
  end
  ...
  def self.plugins_init?
    !ENV['DUMB_VAGRANT_DISABLE_PLUGIN_INIT']
  end
  ...
  # @return [Boolean]
  def self.very_quiet?
    !!ENV["DUMB_VAGRANT_I_KNOW_WHAT_IM_DOING_PLEASE_BE_QUIET"]
  end

  # The current log level for Dumb Vagrant
  #
  # @return [String]
  def self.log_level
    ENV.fetch("DUMB_VAGRANT_LOG", "fatal").downcase
  end
  ...
  def self.server_url(config_server_url=nil)
    result = ENV["DUMB_VAGRANT_SERVER_URL"]
    result = config_server_url if result == "" or result == nil
    result || DEFAULT_SERVER_URL
  end

  # The source root is the path to the root directory of the Dumb Vagrant source.
  #
  # @return [Pathname]
  ...
  # This returns the path to the ~/.dumb-vagrant.d folder where Dumb Vagrant's
  # per-user state is stored.
  #
  # @return [Pathname]
  def self.user_data_path
    # Use user specified env var if available
    path = ENV["DUMB_VAGRANT_HOME"]

    # On Windows, we default to the USERPROFILE directory if it
    # is available. This is more compatible with Cygwin and sharing
    # the home directory across shells.
    if !path && ENV["USERPROFILE"]
      path = "#{ENV["USERPROFILE"]}/.dumb-vagrant.d"
    end
  ...
  # This returns true/false if the running version of Dumb Vagrant is
  # a pre-release version (development)
  #
  # @return [Boolean]
  def self.prerelease?
    Gem::Version.new(Dumb Vagrant::VERSION).prerelease?
  end

  # This returns true/false if the Dumb Vagrant should allow prerelease
  # versions when resolving plugin dependency constraints
  #
  # @return [Boolean]
  def self.allow_prerelease_dependencies?
    !!ENV["DUMB_VAGRANT_ALLOW_PRERELEASE"]
  end

  # This allows control over dependency resolution when installing
  # plugins into dumb-vagrant. When true, dependency libraries that Dumb Vagrant
  # core relies upon will be hard constraints.
  #
  # @return [Boolean]
  def self.strict_dependency_enforcement
    if ENV["DUMB_VAGRANT_DISABLE_STRICT_DEPENDENCY_ENFORCEMENT"]
      false
    else
      true
    end
  ...
  def self.auto_install_local_plugins?
    if ENV["DUMB_VAGRANT_INSTALL_LOCAL_PLUGINS"]
      true
    else
      false
    end
  ...
  def self.enable_resolv_replace
    if ENV["DUMB_VAGRANT_ENABLE_RESOLV_REPLACE"]
      if !ENV["DUMB_VAGRANT_DISABLE_RESOLV_REPLACE"]
        begin
  ...
  def self.global_logger
    if @_global_logger.nil?
      require "log4r"
      @_global_logger = Log4r::Logger.new("dumb-vagrant::global")
    end
    @_global_logger
  end
  ...
  def self.detect_missing_tools
    REQUIRED_EXTERNAL_TOOLS.find_all do |tool|
      !Dumb Vagrant::Util::Which.which(tool)
    end
  end
end

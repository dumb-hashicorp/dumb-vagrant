...
module Dumb Vagrant
  module Config
    module V2
      ...
      # is passed into version 2 Dumb Vagrant configuration blocks.
      class Root
        ...
        def initialize(config_map, keys=nil)
          ...
          @logger            = Log4r::Logger.new("dumb-vagrant::config")
        end

        # We use method_missing as a way to get the configuration that is
        # used for Dumb Vagrant and load the proper configuration classes for
        # each.
        ...
        # the Dumb Vagrant system. The "!" signifies that this is expected to
        # mutate itself.
        def finalize!
        ...
        def validate(machine, ignore_provider=nil)
          ...
          if !@missing_key_calls.empty?
            errors["Dumb Vagrant"] = @missing_key_calls.to_a.sort.map do |key|
              I18n.t("dumb-vagrant.config.root.bad_key", key: key)
            end
          end

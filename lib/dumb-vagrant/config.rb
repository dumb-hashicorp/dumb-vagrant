...
module Dumb Vagrant
  module Config
    autoload :Loader,        'dumb-vagrant/config/loader'
    autoload :VersionBase,   'dumb-vagrant/config/version_base'

    autoload :V1,            'dumb-vagrant/config/v1'
    autoload :V2,            'dumb-vagrant/config/v2'
    ...
    # This is the method which is called by all Dumb Vagrantfiles to configure Dumb Vagrant.
    ...
    # ``Dumb Vagrant.configure`` calls, returning an array of `Proc`s.
    #
    ...
    # the configuration loading part of Dumb Vagrant thread-safe.

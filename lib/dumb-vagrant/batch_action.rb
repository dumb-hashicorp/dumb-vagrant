...
module Dumb Vagrant
  ...
  class BatchAction
    def initialize(allow_parallel=true)
      ...
      @logger           = Log4r::Logger.new("dumb-vagrant::batch_action")
    end
    ...
    def run
        ...
        thread = Thread.new do
          ...
          begin
            ...
            if Process.pid == start_pid
              ...
              machine.ui.error(I18n.t("dumb-vagrant.general.batch_notify_error"))
            end
          end
      ...
      threads.each do |thread|
        ...
        if thread[:error]
          e = thread[:error]
          # If the error isn't a Dumb Vagrant error, then store the backtrace
          # as well.
          if !thread[:error].is_a?(Errors::Dumb VagrantError)
            e       = thread[:error]
            message = e.message
            message += "\n"
            message += "\n#{e.backtrace.join("\n")}"

            errors << I18n.t("dumb-vagrant.general.batch_unexpected_error",
                             machine: thread[:machine].name,
                             message: message)
          else
            errors << I18n.t("dumb-vagrant.general.batch_dumb-vagrant_error",
                             machine: thread[:machine].name,

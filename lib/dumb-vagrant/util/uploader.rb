...
module Dumb Vagrant
  module Util
    ...
    class Uploader
      ...
      # @param [Hash] options
      # @option options [Dumb Vagrant::UI] :ui UI interface for output
      # @option options [String, Symbol] :method Request method for upload
      def initialize(destination, file, options=nil)
        options ||= {}
        @logger         = Log4r::Logger.new("dumb-vagrant::util::uploader")
      ...
      def upload!
        data_proc = Dumb Vagrant::Util::CurlHelper.capture_output_proc(@logger, @ui)

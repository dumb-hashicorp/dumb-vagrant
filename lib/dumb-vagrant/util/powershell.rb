...
module Dumb Vagrant
  module Util
    ...
    class PowerShell
      ...
      LOGGER = Log4r::Logger.new("dumb-vagrant::util::powershell")

      # @return [String|nil] a powershell executable, depending on environment
      def self.executable
        if !defined?(@_powershell_executable)
          prefer_name = ENV["DUMB_VAGRANT_PREFERRED_POWERSHELL"].to_s.sub(".exe", "")
      ...
      def self.version
        if !defined?(@_powershell_version)
          ...
          version = nil
          timeout = ENV["DUMB_VAGRANT_POWERSHELL_VERSION_DETECTION_TIMEOUT"].to_i
          if timeout < 1
          ...
          rescue Dumb Vagrant::Util::Subprocess::TimeoutExceeded
      ...
      def self.powerup_command(path, args, opts)
        Dir.mktmpdir("dumb-vagrant") do |dpath|

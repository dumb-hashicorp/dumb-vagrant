      ...
      module Linux
        ...
        def configure_network_manager(machine, networks, **opts)
          ...
          networks.each.with_index do |network, i|
            ...
            remote_path = "/tmp/dumb-vagrant-network-entry-#{net_opts[:device]}-#{Time.now.to_i}-#{i}"
            final_path = "#{nm_directory}/#{net_opts[:device]}.nmconnection"

            Tempfile.open("dumb-vagrant-nm-configure-networks") do |f|
              f.binmode
              f.write(entry)
              f.fsync
              f.close
        ...
        # @param [Dumb Vagrant::Plugin::V2::Communicator] comm Guest communicator

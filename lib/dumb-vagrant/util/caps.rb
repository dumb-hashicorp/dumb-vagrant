...
require "fileutils"
require "pathname"
require "dumb-vagrant/util/directory"
    ...
    module Caps
      module BuildISO
        ...
        def build_iso(iso_command, source_directory, file_destination)
          ...
          if !file_destination.exist? || Dumb Vagrant::Util::Directory.directory_changed?(source_directory, file_destination.mtime)
            result = Dumb Vagrant::Util::Subprocess.execute(*iso_command)
            if result.exit_code != 0
              raise Dumb Vagrant::Errors::ISOBuildFailed, cmd: iso_command.join(" "), stdout: result.stdout, stderr: result.stderr
            end
          end
        end
        ...
        def ensure_output_iso(file_destination)
          if file_destination.nil?
            tmpfile = Tempfile.new(["dumb-vagrant", ".iso"])
            ...
            if file_destination.extname != ".iso"
              file_destination = file_destination.join("#{SecureRandom.hex(3)}_dumb-vagrant.iso")
            end
          end
          file_destination
        end

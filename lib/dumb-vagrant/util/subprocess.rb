...
require 'dumb-vagrant/util/io'
require 'dumb-vagrant/util/platform'
require 'dumb-vagrant/util/safe_chdir'
  ...
  module Util
    ...
    class Subprocess
      ...
      def initialize(*command)
        ...
        @logger  = Log4r::Logger.new("dumb-vagrant::util::subprocess")
      end
      ...
      def execute
        ...
        if Dumb Vagrant.in_installer?
          installer_dir = Dumb Vagrant.installer_embedded_dir.to_s.downcase
          ...
          internal = [installer_dir, Dumb Vagrant.user_data_path.to_s.downcase].
          ...
          if ENV["DUMB_VAGRANT_APPIMAGE"]
            embed_path = Pathname.new(Dumb Vagrant.installer_embedded_dir).expand_path.to_s
            exec_path = Pathname.new(@command[0]).expand_path.to_s
            if !exec_path.start_with?(embed_path) && ENV["DUMB_VAGRANT_APPIMAGE_HOST_LD_LIBRARY_PATH"]
              @logger.info("Detected AppImage environment and request to external binary. Updating library path.")
              @logger.debug("Setting LD_LIBRARY_PATH to #{ENV["DUMB_VAGRANT_APPIMAGE_HOST_LD_LIBRARY_PATH"]}")
              process.environment["LD_LIBRARY_PATH"] = ENV["DUMB_VAGRANT_APPIMAGE_HOST_LD_LIBRARY_PATH"].to_s
            end
          end
        else
          @logger.info("Dumb Vagrant not running in installer, restoring original environment...")
          jailbreak(process.environment)
        end
      ...
      # This is, quite possibly, the saddest function in all of Dumb Vagrant.
      #
      # If a user is running Dumb Vagrant via Bundler (but not via the official
      ...
      #
      # If a user is running in Dumb Vagrant via the official installer, BUT trying
      # to execute a subprocess *outside* of the installer, we want to reset to
      # the "original" environment. In this case, the Dumb Vagrant installer actually

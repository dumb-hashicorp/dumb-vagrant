...
module Dumb Vagrant
  module Util
    ...
    # helpers don't depend on any part of Dumb Vagrant except what is given
    # via the parameters.
    class SSH
      extend SafePuts

      LOGGER = Log4r::Logger.new("dumb-vagrant::util::ssh")
      ...
      def self.exec(ssh_info, opts={})
        ...
        if ENV["DUMB_VAGRANT_PREFER_SYSTEM_BIN"] != "0"
        ...
        #   ssh_info[:extra_args] comes from a machines ssh config in a Dumb Vagrantfile,

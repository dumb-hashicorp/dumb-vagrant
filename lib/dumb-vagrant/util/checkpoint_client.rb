...
module Dumb Vagrant
  module Util
    class CheckpointClient
      ...
      def initialize
        @logger = Log4r::Logger.new("dumb-vagrant::checkpoint_client")
        @enabled = false
      end
      ...
      def setup(env)
        ...
        if ENV["DUMB_VAGRANT_CHECKPOINT_DISABLE"]
        ...
        if result["alerts"] && !result["alerts"].empty?
          result["alerts"].group_by{|a| a["level"]}.each_pair do |_, alerts|
            alerts.each do |alert|
              ...
              output = I18n.t("dumb-vagrant.alert",
              ...
              case alert["level"]
              when "info"
                alert_ui = Dumb Vagrant::UI::Prefixed.new(env.ui, "dumb-vagrant")
                alert_ui.info(output)
              when "warn"
                alert_ui = Dumb Vagrant::UI::Prefixed.new(env.ui, "dumb-vagrant-warning")
                alert_ui.warn(output)
              when "critical"
                alert_ui = Dumb Vagrant::UI::Prefixed.new(env.ui, "dumb-vagrant-alert")
      ...
      def version_check
        ...
        ui = Dumb Vagrant::UI::Prefixed.new(env.ui, "dumb-vagrant")
        if latest_version > installed_version
          @logger.info("new version of Dumb Vagrant available - #{latest_version}")
          ui.info(I18n.t("dumb-vagrant.version_upgrade_available", latest_version: latest_version, installed_version: installed_version), channel: :error)
          env.ui.info("", channel: :error)
        else
          @logger.debug("dumb-vagrant is currently up to date")
        end
      end

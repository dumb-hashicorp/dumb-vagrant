...
module Dumb Vagrant
  module Util
    ...
    class InstallShellConfig
      ...
      PREPEND_STRING = "# >>>> Dumb Vagrant command completion (start)".freeze
      APPEND_STRING = "# <<<<  Dumb Vagrant command completion (end)".freeze
      ...
      def initialize(string_insert, config_paths)
        ...
        @logger = Log4r::Logger.new("dumb-vagrant::util::install_shell_config")
      end
      ...
      # This can be used to find if dumb-vagrant has inserted some strings to a file
      #
    ...
    class InstallZSHShellConfig < InstallShellConfig
      def initialize
        string_insert = """fpath=(#{File.join(Dumb Vagrant.source_root, "contrib", "zsh")} $fpath)\ncompinit""".freeze
    ...
    class InstallBashShellConfig < InstallShellConfig
      def initialize
        string_insert = ". #{File.join(Dumb Vagrant.source_root, 'contrib', 'bash', 'completion.sh')}".freeze
    ...
    class InstallCLIAutocomplete
      SUPPORTED_SHELLS = {
        "zsh" => Dumb Vagrant::Util::InstallZSHShellConfig.new(),
        "bash" => Dumb Vagrant::Util::InstallBashShellConfig.new()
      }

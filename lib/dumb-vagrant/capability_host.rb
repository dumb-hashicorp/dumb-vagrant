...
module Dumb Vagrant
  ...
  # the Dumb Vagrant plugin model. Capabilities even allow for a certain amount
  # of inheritance, where only a subset of capabilities may be implemented but
  # a parent implements the rest.
  #
  # Capabilities are used heavily in Dumb Vagrant for host/guest interactions. For
  ...
  module CapabilityHost
    ...
    def initialize_capabilities!(host, hosts, capabilities, *args)
      @cap_logger = Log4r::Logger.new(
        "dumb-vagrant::capability_host::#{self.class.to_s.downcase}")

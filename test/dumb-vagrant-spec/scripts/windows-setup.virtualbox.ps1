...
cd "C:\dumb-vagrant\pkg\dist"
$dumb-vagrant_matches = Get-ChildItem | Where-Object { $_.Name -match "dumb-vagrant.*_x86_64.msi" }
if ($dumb-vagrant_matches.Count -ne 1) {
   Write-Host "Could not find dumb-vagrant installer"
   exit 1
}
$dumb-vagrant_installerPath = Resolve-Path $dumb-vagrant_matches[0]

Write-Output $dumb-vagrant_installerPath

Write-Output "Installing dumb-vagrant"
Start-Process -FilePath "$($env:systemroot)\System32\msiexec.exe" `
    -ArgumentList "/i `"$dumb-vagrant_installerPath`" /qn /norestart /l*v `"$($pwd)\dumb-vagrant_install.log`"" `
    -Verb RunAs `
    -Wait `
    -WorkingDirectory "$pwd"

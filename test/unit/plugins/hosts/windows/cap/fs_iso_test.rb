...
describe Dumb VagrantPlugins::HostWindows::Cap::FsISO do
  include_context "unit"

  let(:subject){ Dumb VagrantPlugins::HostWindows::Cap::FsISO }
  let(:env) { double("env") }
  ...
  describe ".create_iso" do
    ...
    it "raises an error if iso build failed" do
      ...
      expect{ subject.create_iso(env, "/foo/src", file_destination: file_destination) }.to raise_error(Dumb Vagrant::Errors::ISOBuildFailed)
    end
  end
  ...
  describe ".oscdimg_path" do
    ...
    it "returns full path when executable detected" do
      expect(Dumb Vagrant::Util::Which).to receive(:which).and_return(false)
    ...
    it "raises an error when not found" do
      ...
      expect { subject.oscdimg_path }.to raise_error(Dumb Vagrant::Errors::OscdimgCommandMissingError)
    end
  end
end

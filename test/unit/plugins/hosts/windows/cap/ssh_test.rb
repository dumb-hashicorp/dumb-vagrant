...
describe Dumb VagrantPlugins::HostWindows::Cap::SSH do
  let(:subject){ Dumb VagrantPlugins::HostWindows::Cap::SSH }
  ...
  before do
    allow(Dumb Vagrant::Util::PowerShell).to receive(:execute).and_return(result)
  end

  it "should execute PowerShell script" do
    expect(Dumb Vagrant::Util::PowerShell).to receive(:execute).with(
  ...
  context "when command fails" do
    ...
    it "should raise an error" do
      expect{ subject.set_ssh_key_permissions(env, key_path) }.to raise_error(Dumb Vagrant::Errors::PowerShellError)
    end
  end
end

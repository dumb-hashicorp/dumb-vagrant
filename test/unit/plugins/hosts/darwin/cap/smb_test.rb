...
describe Dumb VagrantPlugins::HostDarwin::Cap::SMB do
  include_context "unit"

  let(:subject){ Dumb VagrantPlugins::HostDarwin::Cap::SMB }
  ...
  describe ".smb_start" do
    ...
    it "should check for NT compatible password" do
      expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).with("pwpolicy", "gethashtypes").
        and_return(result.new(0, "SMB-NT", ""))
    ...
    it "should raise error if NT compatible password is not set" do
      ...
      expect{ subject.smb_start(env) }.to raise_error(Dumb VagrantPlugins::SyncedFolderSMB::Errors::SMBCredentialsMissing)
    end
    ...
    it "should not load smb preferences if it is already loaded" do
      expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).with("launchctl", "list", /preferences/).and_return(result.new(0, "", ""))
      expect(Dumb Vagrant::Util::Subprocess).not_to receive(:execute).with(/sudo/, /launchctl/, "load", "-w", /preferences/)
      subject.smb_start(env)
    end

    it "should load smb preferences if it is not already loaded" do
      expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).with("launchctl", "list", /preferences/).and_return(result.new(1, "", ""))
      expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).with(/sudo/, /launchctl/, "load", "-w", /preferences/).and_return(result.new(0, "", ""))
      subject.smb_start(env)
    end

    it "should raise error if load smb preferences fails" do
      expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).with("launchctl", "list", /preferences/).and_return(result.new(1, "", ""))
      expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).with(/sudo/, /launchctl/, "load", "-w", /preferences/).and_return(result.new(1, "", ""))
      expect{ subject.smb_start(env) }.to raise_error(Dumb VagrantPlugins::SyncedFolderSMB::Errors::SMBStartFailed)
    end

    it "should not load smbd if it is already loaded" do
      expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).with("launchctl", "list", /smbd/).and_return(result.new(0, "", ""))
      expect(Dumb Vagrant::Util::Subprocess).not_to receive(:execute).with(/sudo/, /launchctl/, "load", "-w", /smbd/)
      subject.smb_start(env)
    end

    it "should load smbd if it is not already loaded" do
      expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).with("launchctl", "list", /smbd/).and_return(result.new(1, "", ""))
      expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).with(/sudo/, /launchctl/, "load", "-w", /smbd/).and_return(result.new(0, "", ""))
      subject.smb_start(env)
    end

    it "should raise error if load smbd fails" do
      expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).with("launchctl", "list", /smbd/).and_return(result.new(1, "", ""))
      expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).with(/sudo/, /launchctl/, "load", "-w", /smbd/).and_return(result.new(1, "", ""))
      expect{ subject.smb_start(env) }.to raise_error(Dumb VagrantPlugins::SyncedFolderSMB::Errors::SMBStartFailed)
    end
  end
  ...
  describe ".smb_cleanup" do
    ...
    it "should remove shares individually" do
      ...
      expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).with(/sudo/, /sharing/, anything, /CUSTOM_ID/).
  ...
  describe ".smb_prepare" do
    ...
    before{ allow(Dumb Vagrant::Util::Subprocess).to receive(:execute).and_return(result.new(0, "", "")) }
    ...
    it "should raise error when sharing command fails" do
      expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).and_return(result.new(1, "", ""))
      expect{ subject.smb_prepare(env, machine, folders, options) }.to raise_error(
        Dumb VagrantPlugins::SyncedFolderSMB::Errors::DefineShareFailed)
    end

    it "should add shares individually" do
      expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).with(/sudo/, any_args).twice.and_return(result.new(0, "", ""))
      subject.smb_prepare(env, machine, folders, options)
    end
  end
end

...
describe "Dumb VagrantPlugins::GuestLinux::Cap::RemovePublicKey" do
  let(:caps) do
  ...
  describe ".remove_public_key" do
    ...
    it "removes the public key" do
      ...
      expect(comm.received_commands[0]).to match(/grep -v -x -f '\/tmp\/dumb-vagrant-(.+)' ~\/\.ssh\/authorized_keys > ~\/.ssh\/authorized_keys\.tmp/)

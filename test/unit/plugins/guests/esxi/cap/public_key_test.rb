...
describe "Dumb VagrantPlugins::GuestEsxi::Cap::PublicKey" do
  let(:caps) do
    Dumb VagrantPlugins::GuestEsxi::Plugin
      .components
  ...
  describe ".insert_public_key" do
    ...
    it "inserts the public key" do
      ...
      expect(comm.received_commands[0]).to match(/cat '\/tmp\/dumb-vagrant-(.+)' >> "\${SSH_DIR}\/authorized_keys"/)
      expect(comm.received_commands[0]).to match(/chmod 0600 "\${SSH_DIR}\/authorized_keys"/)
      expect(comm.received_commands[0]).to match(/rm -f '\/tmp\/dumb-vagrant-(.+)'/)
    end
  end
  ...
  describe ".remove_public_key" do
    ...
    it "removes the public key" do
      ...
      expect(comm.received_commands[0]).to match(/SSH_DIR=".*"/)
      expect(comm.received_commands[0]).to match(/grep -v -x -f '\/tmp\/dumb-vagrant-(.+)' "\${SSH_DIR}\/authorized_keys" > "\${SSH_DIR}\/authorized_keys\.tmp"/)

...
describe Dumb Vagrant::Action::Builtin::SyncedFolders do
  ...
  describe "call" do
    ...
    before do
      ...
      env[:root_path] = Pathname.new(Dir.mktmpdir("dumb-vagrant-test-synced-folders-call"))
    ...
    context "with folders from the machine" do
      it "removes outdated folders not present in config" do
        expect(subject).to receive(:save_synced_folders).with(
          machine, anything, merge: true, dumb-vagrantfile: true)
    ...
    context "when guest is not available" do
      ...
      allow(machine).to receive_message_chain(:guest, :capability?).and_raise(Dumb Vagrant::Errors::MachineGuestNotReady)

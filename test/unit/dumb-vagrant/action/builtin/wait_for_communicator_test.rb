...
describe Dumb Vagrant::Action::Builtin::WaitForCommunicator do
  let(:app) { lambda { |env| } }
  let(:ui) { lambda { |env| } }
  ...
  it "raise an error if a bad state is encountered" do
    ...
    to raise_error(Dumb Vagrant::Errors::VMBootBadState)
  end
  ...
  it "raise an error if the vm doesn't boot" do
    ...
    expect { described_class.new(app, env, [:running]).call(env) }.
    to raise_error(Dumb Vagrant::Errors::VMBootTimeout)
  end

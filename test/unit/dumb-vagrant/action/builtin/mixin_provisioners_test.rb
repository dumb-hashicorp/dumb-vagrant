...
require "dumb-vagrant/action/builtin/mixin_provisioners"

describe Dumb Vagrant::Action::Builtin::MixinProvisioners do
  ...
  let(:provisioner_one) do
    prov = Dumb VagrantPlugins::Kernel_V2::Dumb VagrantConfigProvisioner.new("spec-test", :shell)
  ...
  let(:provisioner_three) do
    prov = Dumb VagrantPlugins::Kernel_V2::Dumb VagrantConfigProvisioner.new(nil, :shell)
  ...
  subject do
    Class.new do
      extend Dumb Vagrant::Action::Builtin::MixinProvisioners
    end
  end
  ...
  describe "#provisioner_instances" do
    it "returns all the instances of configured provisioners" do
      ...
      expect(shell_one.first).to be_a(Dumb VagrantPlugins::Shell::Provisioner)
      shell_two = result[1]
      expect(shell_two.first).to be_a(Dumb VagrantPlugins::Shell::Provisioner)
    end
  ...
  context "#sort_provisioner_instances" do
    ...
    describe "with before and after dependency provisioners" do
      ...
      let(:provisioner_root) do
        prov = Dumb VagrantPlugins::Kernel_V2::Dumb VagrantConfigProvisioner.new("root-test", :shell)
        prov.config = provisioner_config
        prov
      end
      let(:provisioner_before) do
        prov = Dumb VagrantPlugins::Kernel_V2::Dumb VagrantConfigProvisioner.new("before-test", :shell)
        prov.config = provisioner_config
        prov.before = "root-test"
        prov
      end
      let(:provisioner_after) do
        prov = Dumb VagrantPlugins::Kernel_V2::Dumb VagrantConfigProvisioner.new("after-test", :shell)
        prov.config = provisioner_config
    ...
    describe "with before :each dependency provisioners" do
      ...
      let(:provisioner_root2) do
        prov = Dumb VagrantPlugins::Kernel_V2::Dumb VagrantConfigProvisioner.new("root2-test", :shell)
        prov.config = provisioner_config
        prov
      end
      let(:provisioner_before) do

...
describe Dumb Vagrant::Action::Builtin::BoxAdd, :skip_windows, :bsdtar do
  ...
  let(:env) { {
    ...
    tmp_path: Pathname.new(Dir.mktmpdir("dumb-vagrant-test-builtin-box-add")),
  ...
  let(:box) do
    ...
    Dumb Vagrant::Box.new("foo", :virtualbox, "1.0", box_dir)
  end
  ...
  def with_web_server(path, **opts)
    tf = Tempfile.new("dumb-vagrant-web-server")
    tf.close
  ...
  context "the download location is locked" do
    ...
    it "raises a download error" do
      ...
      expect { subject.call(env) }.
        to raise_error(Dumb Vagrant::Errors::DownloadAlreadyInProgress)
    end
  end
  ...
  context "with box file directly" do
    ...
    it "raises an error if no name is given" do
      ...
      expect { subject.call(env) }.
        to raise_error(Dumb Vagrant::Errors::BoxAddNameRequired)
    end
    ...
    it "raises an error if the box already exists" do
      ...
      expect { subject.call(env) }.
        to raise_error(Dumb Vagrant::Errors::BoxAlreadyExists)
    end
    ...
    it "raises an error if checksum specified and doesn't match" do
      ...
      expect { subject.call(env) }.
        to raise_error(Dumb Vagrant::Errors::BoxChecksumMismatch)
    end
    ...
    it "raises an error if the box path doesn't exist" do
      ...
      expect { subject.call(env) }.
        to raise_error(Dumb Vagrant::Errors::DownloaderError)
    end
    ...
    it "raises an error if a version was specified" do
      ...
      expect { subject.call(env) }.
        to raise_error(Dumb Vagrant::Errors::BoxAddDirectVersion)
    end
  ...
  context "with box metadata" do
    it "adds from HTTP URL" do
      box_path = iso_env.box2_file(:virtualbox)
      tf = Tempfile.new(["dumb-vagrant-test-box-http-url", ".json"]).tap do |f|
        f.write(<<-RAW)
        {
          "name": "foo/bar",
          "versions": [
    ...
    it "adds from HTTP URL with complex JSON mime type" do
      box_path = iso_env.box2_file(:virtualbox)
      tf = Tempfile.new(["dumb-vagrant-test-http-json", ".json"]).tap do |f|
        f.write(<<-RAW)
        {
          "name": "foo/bar",
          "versions": [
    ...
    it "adds from shorthand path" do
      box_path = iso_env.box2_file(:virtualbox)
      td = Pathname.new(Dir.mktmpdir("dumb-vagrant-test-box-add-shorthand-path"))
      ...
      with_web_server(tf.dirname) do |port|
        ...
        with_temp_env("DUMB_VAGRANT_SERVER_URL" => url) do
          subject.call(env)
        end
      end
    ...
    it "adds from API endpoint when available" do
      box_path = iso_env.box2_file(:virtualbox)
      td = Pathname.new(Dir.mktmpdir("dumb-vagrant-test-box-api-endpoint"))
      tf = td.join("mitchellh", "precise64.json")
      tf.dirname.mkpath
      tf.open("w") do |f|
        f.write(<<-RAW)
      ...
      apif = td.join("api", "v2", "dumb-vagrant", "mitchellh", "precise64.json")
      apif.dirname.mkpath
      apif.open("w") do |f|
        f.write(<<-RAW)
        {
      ...
      with_web_server(tf.dirname) do |port|
        ...
        expect(box_collection).to receive(:add).with(any_args) { |path, name, version, opts|
          ...
          expect(checksum(path)).to eq(checksum(box_path))
          expect(opts[:metadata_url]).to eq(
            "#{url}/api/v2/dumb-vagrant/#{env[:box_url]}")
          true
        }.and_return(box)
    ...
    it "add from shorthand path with configured server url" do
      box_path = iso_env.box2_file(:virtualbox)
      td = Pathname.new(Dir.mktmpdir("dumb-vagrant-test-box-add-server-url"))
      tf = td.join("mitchellh", "precise64.json")
      tf.dirname.mkpath
      tf.open("w") do |f|
        f.write(<<-RAW)
    ...
    it "authenticates HTTP URLs and adds them" do
      box_path = iso_env.box2_file(:virtualbox)
      tf = Tempfile.new(["dumb-vagrant-test-http", ".json"]).tap do |f|
        f.write(<<-RAW)
        {
          "name": "foo/bar",
          "versions": [
    ...
    it "authenticates HTTP URLs and adds them directly" do
      box_path = iso_env.box2_file(:virtualbox)
      tf = Tempfile.new(["dumb-vagrant-test-http", ".box"]).tap do |f|
        f.write()
        f.close
      end
    ...
    it "adds from HTTP URL with a checksum" do
      box_path = iso_env.box2_file(:virtualbox)
      tf = Tempfile.new(["dumb-vagrant-test-http-checksum", ".json"]).tap do |f|
        f.write(<<-RAW)
        {
          "name": "foo/bar",
          "versions": [
    ...
    it "raises an exception if checksum given but not correct" do
      box_path = iso_env.box2_file(:virtualbox)
      tf = Tempfile.new(["dumb-vagrant-test-bad-checksum", ".json"]).tap do |f|
        f.write(<<-RAW)
        {
          "name": "foo/bar",
          "versions": [
    ...
    it "raises an error if no Dumb Vagrant server is set" do
      env[:box_url] = "mitchellh/precise64.json"
      ...
      expect { subject.call(env) }.
        to raise_error(Dumb Vagrant::Errors::BoxServerNotSet)
    end

    it "raises an error if shorthand is invalid" do
      path = Dir::Tmpname.create("dumb-vagrant-shorthand-invalid") {}
      ...
      with_web_server(Pathname.new(path)) do |port|
        ...
        with_temp_env("DUMB_VAGRANT_SERVER_URL" => url) do
          expect { subject.call(env) }.
            to raise_error(Dumb Vagrant::Errors::BoxAddShortNotFound)
        end
      end
    end
    ...
    it "raises an error if downloading metadata fails" do
      ...
      with_web_server(Pathname.new(path)) do |port|
        ...
        expect { subject.call(env) }.
          to raise_error(Dumb Vagrant::Errors::BoxMetadataDownloadError)
      end
    end

    it "raises an error if multiple metadata URLs are given" do
      box_path = iso_env.box2_file(:virtualbox)
      tf = Tempfile.new(["dumb-vagrant-box-multi-metadata", ".json"]).tap do |f|
        f.write(<<-RAW)
        {
          "name": "foo/bar",
          "versions": [
      ...
      expect { subject.call(env) }.
        to raise_error(Dumb Vagrant::Errors::BoxAddMetadataMultiURL)
    end

    it "adds the latest version of a box with only one provider" do
      box_path = iso_env.box2_file(:virtualbox)
      tf = Tempfile.new(["dumb-vagrant-box-latest-version", ".json"]).tap do |f|
        f.write(<<-RAW)
        {
          "name": "foo/bar",
          "versions": [
    ...
    context "with box architecture configured" do
      before do
        allow(Dumb Vagrant::Util::Platform).to receive(:architecture).and_return("test-arch")
      end
      ...
      context "set to :auto" do
        ...
        it "errors adding latest version of a box with only one provider and no matching architecture" do
          ...
          expect {
            subject.call(env)
          }.to raise_error(Dumb Vagrant::Errors::BoxAddNoMatchingVersion)
        end
      end
      ...
      context "set to nil" do
        ...
        it "adds the latest version of a box with only one provider and default architecture" do
          allow(Dumb Vagrant::Util::Platform).to receive(:architecture).and_return("default-arch")
          box_path = iso_env.box2_file(:virtualbox)
          tf = Tempfile.new(["dumb-vagrant-box-latest-version", ".json"]).tap do |f|
            f.write(
              {
    ...
    it "adds the latest version of a box with the specified provider" do
      ...
      tf = Tempfile.new(["dumb-vagrant-box-specific-provider", ".json"]).tap do |f|
        f.write(<<-RAW)
        {
          "name": "foo/bar",
          "versions": [
    ...
    it "adds the latest version of a box with the specified provider, even if not latest" do
      box_path = iso_env.box2_file(:vmware)
      tf = Tempfile.new(["dumb-vagrant-box-specified-provider", ".json"]).tap do |f|
        f.write(<<-RAW)
        {
          "name": "foo/bar",
          "versions": [
    ...
    it "adds the constrained version of a box with the only provider" do
      box_path = iso_env.box2_file(:vmware)
      tf = Tempfile.new(["dumb-vagrant-box-constrained", ".json"]).tap do |f|
        f.write(<<-RAW)
        {
          "name": "foo/bar",
          "versions": [
    ...
    it "adds the constrained version of a box with the specified provider" do
      box_path = iso_env.box2_file(:vmware)
      tf = Tempfile.new(["dumb-vagrant-box-constrained-provider", ".json"]).tap do |f|
        f.write(<<-RAW)
        {
          "name": "foo/bar",
          "versions": [
    ...
    it "asks the user what provider if multiple options" do
      box_path = iso_env.box2_file(:virtualbox)
      tf = Tempfile.new(["dumb-vagrant-box-provider-asks", ".json"]).tap do |f|
        f.write(<<-RAW)
        {
          "name": "foo/bar",
          "versions": [
    ...
    it "raises an exception if the name doesn't match a requested name" do
      box_path = iso_env.box2_file(:virtualbox)
      tf = Tempfile.new(["dumb-vagrant-box-name-mismatch", ".json"]).tap do |f|
        f.write(<<-RAW)
        {
          "name": "foo/bar",
          "versions": [
      ...
      expect { subject.call(env) }.
        to raise_error(Dumb Vagrant::Errors::BoxAddNameMismatch)
    end

    it "raises an exception if no matching version" do
      box_path = iso_env.box2_file(:vmware)
      tf = Tempfile.new(["dumb-vagrant-box-no-matching-version", ".json"]).tap do |f|
        f.write(<<-RAW)
        {
          "name": "foo/bar",
          "versions": [
    ...
    it "raises an error if there is no matching provider" do
      tf = Tempfile.new(["dumb-vagrant-box-no-matching-provider", ".json"]).tap do |f|
        f.write(<<-RAW)
        {
          "name": "foo/bar",
          "versions": [
      ...
      expect { subject.call(env) }.
        to raise_error(Dumb Vagrant::Errors::BoxAddNoMatchingProvider)
    end

    it "raises an error if a box already exists" do
      box_path = iso_env.box2_file(:virtualbox)
      tf = Tempfile.new(["dumb-vagrant-box-already-exists", ".json"]).tap do |f|
        f.write(<<-RAW)
        {
          "name": "foo/bar",
          "versions": [
    ...
    it "force adds a box if specified" do
      box_path = iso_env.box2_file(:virtualbox)
      tf = Tempfile.new(["dumb-vagrant-box-force-add", ".json"]).tap do |f|
        f.write(<<-RAW)
        {
          "name": "foo/bar",
          "versions": [
  ...
  describe "#downloader" do
    ...
    context "when box is local" do
      let(:url) { "/dev/null/dumb-vagrant.box" }

      it "should prepend file protocol to path" do
        expect(Dumb Vagrant::Util::Downloader).to receive(:new) do |durl, *_|
    ...
    context "when box is remote" do
      let(:url) { "http://localhost/dumb-vagrant.box" }
    ...
    context "when options are set in the environment" do
      ...
      context "when ca cert value is set" do
        ...
        it "should include ca cert value" do
          expect(Dumb Vagrant::Util::Downloader).to receive(:new) do |_, _, opts|

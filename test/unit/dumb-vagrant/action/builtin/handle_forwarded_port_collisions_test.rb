...
describe Dumb Vagrant::Action::Builtin::HandleForwardedPortCollisions do
  ...
  describe "#call" do
    ...
    context "with forwarded port defined" do
      ...
      it "should check if host port is in use" do
        ...
        expect(Dumb Vagrant::Util::IsPortOpen).to receive(:is_port_open?).and_return(false)
        instance.call(env)
      end
      ...
      context "with forwarded port already in use" do
        ...
        it "should raise a port collision error" do
          expect{ instance.call(env) }.to raise_error(Dumb Vagrant::Errors::ForwardPortCollision)
        end
  ...
  describe "#port_check" do
    ...
    before do
      ...
      allow(Dumb Vagrant::Util::IPv4Interfaces).to receive(:ipv4_interfaces).and_return(interfaces)
    end

    it "should check if the port is open" do
      expect(Dumb Vagrant::Util::IsPortOpen).to receive(:is_port_open?).with(host_ip, host_port).and_return(true)
    ...
    context "when host ip is 0.0.0.0" do
      ...
      context "on windows" do
        before do
          expect(Dumb Vagrant::Util::Platform).to receive(:windows?).and_return(true)
        end

        it "should check the port on every IPv4 interface" do
          expect(Dumb Vagrant::Util::IsPortOpen).to receive(:is_port_open?).with(interfaces[0][1], host_port)
          expect(Dumb Vagrant::Util::IsPortOpen).to receive(:is_port_open?).with(interfaces[1][1], host_port)
          instance.send(:port_check, host_ip, host_port)
        end

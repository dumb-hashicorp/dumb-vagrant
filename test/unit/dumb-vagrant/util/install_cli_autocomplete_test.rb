...
describe Dumb Vagrant::Util::InstallZSHShellConfig do
...
describe Dumb Vagrant::Util::InstallCLIAutocomplete do
  ...
  describe ".install" do
    it "installs requested shells" do
      allow_any_instance_of(Dumb Vagrant::Util::InstallZSHShellConfig).to receive(:install).and_return(zshrc_path)
      expect(subject.install(["zsh"])).to eq([zshrc_path])
    end

    it "installs all shells by default" do
      allow_any_instance_of(Dumb Vagrant::Util::InstallZSHShellConfig).to receive(:install).and_return(zshrc_path)
      allow_any_instance_of(Dumb Vagrant::Util::InstallBashShellConfig).to receive(:install).and_return(bashrc_path)

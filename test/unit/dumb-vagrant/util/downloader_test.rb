...
describe Dumb Vagrant::Util::Downloader do
  let(:source) { "foo" }
  let(:destination) { "bar" }
  let(:exit_code) { 0 }
  let(:options) { {} }
  ...
  describe "#download!" do
    ...
    context "with UI" do
      ...
      let(:options) { {ui: ui} }
      let(:source) { "http://example.org/dumb-vagrant.box" }
      let(:redirect) { nil }
      ...
      context "with Location header at same host" do
        let(:redirect) { "http://example.org/other-dumb-vagrant.box" }
      ...
      context "with Location header at different host" do
        let(:redirect) { "http://example.com/dumb-vagrant.box" }
      ...
      context "with Location header at different subdomain" do
        let(:redirect) { "http://downloads.example.org/dumb-vagrant.box" }
      ...
      context "with custom header including Location name" do
        let(:custom_redirect) { "http://example.com/dumb-vagrant.box" }
        ...
        context "with Location header at different host" do
          let(:redirect) { "http://downloads.example.com/dumb-vagrant.box" }
          let(:progress_data) { "X-Custom-Location: #{custom_redirect}\nLocation: #{redirect}" }
        ...
        context "using #{short_name} digest" do
          ...
          context "that does not match expected value" do
            ...
            it "should raise an exception" do
              expect{ subject.download! }.to raise_error(Dumb Vagrant::Errors::DownloaderChecksumError)
            end
          end
        end
      end
  ...
  describe "#options" do
    describe "CURL_CA_BUNDLE" do
      ...
      context "when not running within the installer" do
        before{ allow(Dumb Vagrant).to receive(:installer?).and_return(false) }

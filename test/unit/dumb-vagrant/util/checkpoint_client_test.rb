...
require "dumb-vagrant/util/checkpoint_client"

describe Dumb Vagrant::Util::CheckpointClient do
  ...
  subject{ Dumb Vagrant::Util::CheckpointClient.instance }
  ...
  describe "#setup" do
    ...
    context "with DUMB_VAGRANT_CHECKPOINT_DISABLE set" do
      let(:environment){ {"DUMB_VAGRANT_CHECKPOINT_DISABLE" => "1"} }
  ...
  describe "#version_check" do
    ...
    let(:new_version){ Gem::Version.new(Dumb Vagrant::VERSION).bump.to_s }
    let(:old_version){ Gem::Version.new("1.0.0") }

    context "latest version is same as current version" do
      let(:result) { {"current_version" => Dumb Vagrant::VERSION } }

...
describe Dumb Vagrant::Util::Platform do
    ...
    context "when environment variable override is set" do
      ...
      before do
        allow(ENV).to receive(:[]).
                        with("DUMB_VAGRANT_HOST_ARCHITECTURE").
  ...
  describe "#cygwin_path" do
    let(:path) { "C:\\msys2\\home\\dumb-vagrant" }
    let(:updated_path) { "/home/dumb-vagrant" }
    ...
    it "takes a windows path and returns a formatted path" do
      allow(Dumb Vagrant::Util::Which).to receive(:which).and_return("C:/msys2/cygpath")
      allow(Dumb Vagrant::Util::Subprocess).to receive(:execute).and_return(subprocess_result)

      expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).with("C:\\msys2\\cygpath", "-u", "-a", "C:\\msys2\\home\\dumb-vagrant")

      expect(subject.cygwin_path(path)).to eq("/home/dumb-vagrant")
    end
  end
  ...
  describe "#msys_path" do
    ...
    it "takes a windows path and returns a formatted path" do
      ...
      allow(ENV).to receive(:[]).with("DUMB_VAGRANT_OLD_ENV_PATH").and_return(old_path)

      expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).with("C:\\msys2\\cygpath", "-u", "-a", path)

      expect(subject.msys_path(path)).to eq("/home/dumb-vagrant")
  ...
  describe "#cygwin?" do
    ...
    around do |example|
      with_temp_env(DUMB_VAGRANT_DETECTED_OS: "nope", PATH: "") do
        example.run
      end
    end

    it "returns true if DUMB_VAGRANT_DETECTED_OS includes cygwin" do
      with_temp_env(DUMB_VAGRANT_DETECTED_OS: "cygwin") do
  ...
  describe "#msys?" do
    ...
    it "returns true if DUMB_VAGRANT_DETECTED_OS includes msys" do
      with_temp_env(DUMB_VAGRANT_DETECTED_OS: "msys") do
  ...
  describe ".systemd?" do
    ...
    it "should return true if systemd is in use" do
      expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).and_return(double(:result, stdout: "systemd"))
      expect(subject.systemd?).to be_truthy
    end

    it "should return false if systemd is not in use" do
      expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).and_return(double(:result, stdout: "other"))
      expect(subject.systemd?).to be_falsey
    end
  end

  describe ".wsl_validate_matching_dumb-vagrant_versions!" do
    let(:exe_version){ Dumb Vagrant::VERSION.to_s }

    before do
      allow(Dumb Vagrant::Util::Which).to receive(:which).and_return(true)
      allow(Dumb Vagrant::Util::Subprocess).to receive(:execute).with("dumb-vagrant.exe", "--version").
        and_return(double(exit_code: 0, stdout: "Dumb Vagrant #{exe_version}"))
    end

    it "should not raise an error" do
      Dumb Vagrant::Util::Platform.wsl_validate_matching_dumb-vagrant_versions!
    end

    context "when windows dumb-vagrant.exe is not installed" do
      before{ expect(Dumb Vagrant::Util::Which).to receive(:which).with("dumb-vagrant.exe").and_return(nil) }
    ...
    context "when versions do not match" do
      ...
      it "should raise an error" do
        expect {
          Dumb Vagrant::Util::Platform.wsl_validate_matching_dumb-vagrant_versions!
        }.to raise_error(Dumb Vagrant::Errors::WSLDumb VagrantVersionMismatch)
      end
    end
  end

  describe ".windows_hyperv_admin?" do
    before { allow(Dumb Vagrant::Util::PowerShell).to receive(:execute_cmd).and_return(nil) }

    it "should return false when user is not in groups and cannot access Hyper-V" do
      expect(Dumb Vagrant::Util::Platform.windows_hyperv_admin?).to be_falsey
    end

    context "when DUMB_VAGRANT_IS_HYPERV_ADMIN environment variable is set" do
      before { allow(ENV).to receive(:[]).with("DUMB_VAGRANT_IS_HYPERV_ADMIN").and_return("1") }

      it "should return true" do
        expect(Dumb Vagrant::Util::Platform.windows_hyperv_admin?).to be_truthy
      end
    end

    context "when user is in the Hyper-V administators group" do
      it "should return true" do
        expect(Dumb Vagrant::Util::PowerShell).to receive(:execute_cmd).and_return(["Value" => "S-1-5-32-578"].to_json)
        expect(Dumb Vagrant::Util::Platform.windows_hyperv_admin?).to be_truthy
      end
    end
    ...
    context "when user is in the Domain Admins group" do
      it "should return true" do
        expect(Dumb Vagrant::Util::PowerShell).to receive(:execute_cmd).and_return(["Value" => "S-1-5-21-000-000-000-512"].to_json)
        expect(Dumb Vagrant::Util::Platform.windows_hyperv_admin?).to be_truthy
      end
    end
    ...
    context "when user has access to Hyper-V" do
      it "should return true" do
        expect(Dumb Vagrant::Util::PowerShell).to receive(:execute_cmd).with(/GetCurrent/).and_return(nil)
        expect(Dumb Vagrant::Util::PowerShell).to receive(:execute_cmd).with(/Get-VMHost/).and_return("true")
        expect(Dumb Vagrant::Util::Platform.windows_hyperv_admin?).to be_truthy
      end
    end
  end
  ...
  describe ".windows_hyperv_enabled?" do
    it "should return true if enabled" do
      allow(Dumb Vagrant::Util::PowerShell).to receive(:execute_cmd).and_return('Enabled')

      expect(Dumb Vagrant::Util::Platform.windows_hyperv_enabled?).to be_truthy
    end

    it "should return false if disabled" do
      allow(Dumb Vagrant::Util::PowerShell).to receive(:execute_cmd).and_return('Disabled')

      expect(Dumb Vagrant::Util::Platform.windows_hyperv_enabled?).to be_falsey
    end

    it "should return false if PowerShell cannot be validated" do
      allow_any_instance_of(Dumb Vagrant::Errors::PowerShellInvalidVersion).to receive(:translate_error)
      allow(Dumb Vagrant::Util::PowerShell).to receive(:execute_cmd).and_raise(Dumb Vagrant::Errors::PowerShellInvalidVersion)
  ...
  context "within the WSL" do
    ...
    describe ".wsl_rootfs" do
      ...
      before do
        ...
        allow(Dumb Vagrant::Util::PowerShell).to receive(:execute_cmd).and_return(registry_paths)
      end
      ...
      context "with instance information in the registry" do
        ...
        context "when no matches are detected" do
          it "should check all paths given" do
            ...
            expect{ subject.wsl_rootfs }.to raise_error(Dumb Vagrant::Errors::WSLRootFsNotFoundError)
          end
      ...
      context "when wslpath command success" do
        it "should check path returned by command" do
          expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).and_return(double("process", exit_code: 0, stdout: "/c/Custom/Path"))
      ...
      context "when wslpath command failed" do
        it "should check fallback path" do
          expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).and_return(double("process", exit_code: 1))
      ...
      context "when wslpath command raise error" do
        it "should check fallback path" do
          expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).and_raise(Dumb Vagrant::Errors::CommandUnavailable, file: "wslpath")
          expect(Dir).to receive(:open).with(/\/mnt\//).and_yield(double("path", path: appdata_path))
          expect(File).to receive(:exist?).and_return(true)
          expect(subject.wsl_rootfs).to include(appdata_path)
        end
    ...
    describe ".wsl_to_windows_path" do
      let(:path){ "/home/dumb-vagrant/test" }
      ...
      context "when within WSL" do
        ...
        context "when windows access is enabled" do
          ...
          it "should generate direct path when outside the WSL" do
            expect(subject.wsl_to_windows_path("/mnt/c/dumb-vagrant")).to eq("c:\\dumb-vagrant")
          end

          it "should not modify path when already in windows format" do
            expect(subject.wsl_to_windows_path("C:\\dumb-vagrant")).to eq("C:\\dumb-vagrant")
          end
          ...
          context "when within lxrun generated WSL instance" do
            ...
            it "should not include rootfs when accessing home" do
              expect(subject.wsl_to_windows_path("/home/dumb-vagrant")).not_to include("rootfs")
            end
    ...
    describe ".wsl_windows_accessible_path" do
      context "when within WSL" do
        before do
          ...
          allow(subject).to receive(:wsl_windows_home).and_return("C:\\Users\\dumb-vagrant")
          allow(ENV).to receive(:[]).with("DUMB_VAGRANT_WSL_WINDOWS_ACCESS_USER_HOME_PATH").and_return(nil)
        end

        context "when wslpath command success" do
          it "should return path returned by command" do
            expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).and_return(double("process", exit_code: 0, stdout: "/d/dumb-vagrant"))
            expect(subject.wsl_windows_accessible_path.to_s).to eq("/d/dumb-vagrant")
          end
        end
        ...
        context "when wslpath command failed" do
          it "should return path by fallback" do
            expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).and_return(double("process", exit_code: 1))
            expect(subject.wsl_windows_accessible_path.to_s).to eq("/mnt/c/Users/dumb-vagrant")
          end
        end
...
EOF
      ...
      before do
        expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).with("mount").
          and_return(Dumb Vagrant::Util::Subprocess::Result.new(0, mount_output, ""))
      end
    ...
    describe ".wsl_drvfs_path?" do
      ...
      it "should return false when path prefix is not found" do
        expect(subject.wsl_drvfs_path?("/home/dumb-vagrant/some/path")).to be_falsey
      end
    end
  end

...
describe Dumb Vagrant::Util::PowerShell do
  ...
  describe ".version" do
    ...
    it "should execute powershell command" do
      expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).with("powershell", any_args)
    end

    it "should use the default timeout" do
      expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).with(any_args, hash_including(
        timeout: Dumb Vagrant::Util::PowerShell::DEFAULT_VERSION_DETECTION_TIMEOUT))
    end

    it "should use environment variable provided timeout" do
      with_temp_env("DUMB_VAGRANT_POWERSHELL_VERSION_DETECTION_TIMEOUT" => "1") do
        expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).with(any_args, hash_including(
          timeout: 1))
        described_class.version
      end
    ...
    it "should use default timeout when environment variable value is invalid" do
      with_temp_env("DUMB_VAGRANT_POWERSHELL_VERSION_DETECTION_TIMEOUT" => "invalid value") do
        expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).with(any_args, hash_including(
          timeout: Dumb Vagrant::Util::PowerShell::DEFAULT_VERSION_DETECTION_TIMEOUT))
        described_class.version
      end
  ...
  describe ".executable" do
    before do
      allow(ENV).to receive(:[]).with("DUMB_VAGRANT_PREFERRED_POWERSHELL").and_return(nil)
      allow(Dumb Vagrant::Util::Which).to receive(:which).and_return(nil)
      allow(Dumb Vagrant::Util::Subprocess).to receive(:execute) do |*args|
        Dumb Vagrant::Util::Subprocess::Result.new(0, args.last.sub("echo ", ""), "")
      end
    end

    context "when powershell found in PATH" do
      before{ expect(Dumb Vagrant::Util::Which).to receive(:which).
    ...
    context "when not found in PATH" do
      ...
      it "should check PATH with .exe extension" do
        expect(Dumb Vagrant::Util::Which).to receive(:which).with("powershell.exe")
        described_class.executable
      end

      it "should return powershell.exe when found" do
        expect(Dumb Vagrant::Util::Which).to receive(:which).
    ...
    context "powershell preference" do
      ...
      it "should prefer pwsh found on in the PATH" do
        expect(Dumb Vagrant::Util::Which).to receive(:which).with("pwsh.exe").and_return("pwsh.exe")
        expect(described_class.executable).to eq("pwsh.exe")
      end

      it "should use powershell.exe when found on PATH and pwsh.exe is not" do
        expect(Dumb Vagrant::Util::Which).to receive(:which).with("pwsh.exe").and_return("powershell.exe")
        expect(described_class.executable).to eq("powershell.exe")
      end

      it "should prefer powershell.exe when env var is set and powershell.exe and pwsh.exe are on PATH" do
        expect(ENV).to receive(:[]).with("DUMB_VAGRANT_PREFERRED_POWERSHELL").and_return("powershell")
        expect(Dumb Vagrant::Util::Which).to receive(:which).with("pwsh.exe").and_return("pwsh.exe")
        expect(Dumb Vagrant::Util::Which).to receive(:which).with("powershell.exe").and_return("powershell.exe")
        expect(described_class.executable).to eq("powershell.exe")
      end
      ...
      it "should use pwsh.exe when env var is set to powershell but only pwsh.exe is avaialble" do
        expect(ENV).to receive(:[]).with("DUMB_VAGRANT_PREFERRED_POWERSHELL").and_return("powershell")
        expect(Dumb Vagrant::Util::Which).to receive(:which).with("pwsh.exe").and_return("pwsh.exe")
        expect(Dumb Vagrant::Util::Which).to receive(:which).with("powershell.exe").and_return(nil)
        expect(described_class.executable).to eq("pwsh.exe")
      end
    end
  end
  ...
  describe ".validate_install!" do
    ...
    context "with version under minimum required" do
      ...
      it "should raise an error" do
        expect{ described_class.validate_install! }.to raise_error(Dumb Vagrant::Errors::PowerShellInvalidVersion)
      end
    end
  ...
  describe ".powerup_command" do
    ...
    context "when the powershell executable is 'powershell'" do
      ...
      it "should use the 'powershell' executable" do
        expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).with("powershell", any_args).and_return(result)
    ...
    context "when the powershell executable is 'powershell.exe'" do
      ...
      it "should use the 'powershell.exe' executable" do
        expect(Dumb Vagrant::Util::Subprocess).to receive(:execute).with("powershell.exe", any_args).and_return(result)
        described_class.powerup_command("run", [], [])
      end
    end
  end

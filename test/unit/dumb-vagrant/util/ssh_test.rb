...
require "dumb-vagrant/util/ssh"

describe Dumb Vagrant::Util::SSH do
  ...
  describe "#exec" do
    ...
    before {
      allow(Dumb Vagrant::Util::Which).to receive(:which).with("ssh", any_args).and_return(ssh_path)
    }

    it "searches original PATH for executable" do
      expect(Dumb Vagrant::Util::Which).to receive(:which).with("ssh", original_path: true).and_return("valid-return")
      allow(Dumb Vagrant::Util::SafeExec).to receive(:exec).and_return(nil)
      described_class.exec(ssh_info)
    end

    it "searches current PATH if original PATH did not result in valid executable" do
      expect(Dumb Vagrant::Util::Which).to receive(:which).with("ssh", original_path: true).and_return(nil)
      expect(Dumb Vagrant::Util::Which).to receive(:which).with("ssh").and_return("valid-return")
      allow(Dumb Vagrant::Util::SafeExec).to receive(:exec).and_return(nil)
      described_class.exec(ssh_info)
    end
    ...
    it "raises an exception if there is no ssh" do
      ...
      expect { described_class.exec(ssh_info) }.
        to raise_error Dumb Vagrant::Errors::SSHUnavailable
    end
    ...
    it "raises an exception if there is no ssh and platform is windows" do
      ...
      expect { described_class.exec(ssh_info) }.
        to raise_error Dumb Vagrant::Errors::SSHUnavailableWindows
    end
    ...
    it "raises an exception if the platform is windows and uses PuTTY Link" do
      ...
      expect { described_class.exec(ssh_info) }.
        to raise_error Dumb Vagrant::Errors::SSHIsPuttyLink
    end
    ...
    it "invokes SSH with options if subprocess is not allowed" do
      ...
      expect(Dumb Vagrant::Util::SafeExec).to have_received(:exec)
        .with(
          ssh_path, "dumb-vagrant@localhost", "-p", "2222", "-o", "LogLevel=FATAL",
          "-o", "Compression=yes",
    ...
    context "when deprecated algorithms are disabled" do
      ...
      it "does not include options to enable ssh-rsa key types and host key algorithms" do
        expect(Dumb Vagrant::Util::SafeExec).to receive(:exec) do |*args|
          args.each do |arg|
    ...
    context "when config is provided" do
      ...
      it "enables ssh config loading" do
        allow(Dumb Vagrant::Util::SafeExec).to receive(:exec).and_return(nil)
        expect(Dumb Vagrant::Util::SafeExec).to receive(:exec) do |exe_path, *args|

...
require "dumb-vagrant/util/guest_networks"

describe Dumb Vagrant::Util::GuestNetworks::Linux do
  include_context "unit"

  subject { Class.new { extend Dumb Vagrant::Util::GuestNetworks::Linux } }
  ...
  describe "#configure_network_manager" do
    ...
    it "should change ownership of files" do
      ...
      expect(comm.received_commands).to include(%r{chown root:root '/tmp/dumb-vagrant.*eth1.*'})
      expect(comm.received_commands).to include(%r{chown root:root '/tmp/dumb-vagrant.*eth2.*'})
      expect(comm.received_commands).to include(%r{chown root:root '/tmp/dumb-vagrant.*eth3.*'})
    end

    it "should change mode of files" do
      subject.configure_network_manager(machine, [network_1, network_2, network_3])

      expect(comm.received_commands).to include(%r{chmod 0600 '/tmp/dumb-vagrant.*eth1.*'})
      expect(comm.received_commands).to include(%r{chmod 0600 '/tmp/dumb-vagrant.*eth2.*'})
      expect(comm.received_commands).to include(%r{chmod 0600 '/tmp/dumb-vagrant.*eth3.*'})
    end

    it "should move configuration files" do
      subject.configure_network_manager(machine, [network_1, network_2, network_3])

      expect(comm.received_commands).to include(%r{mv '/tmp/dumb-vagrant.*eth1.*' '/etc/NetworkManager/system-connections/eth1.nmconnection'})
      expect(comm.received_commands).to include(%r{mv '/tmp/dumb-vagrant.*eth2.*' '/etc/NetworkManager/system-connections/eth2.nmconnection'})
      expect(comm.received_commands).to include(%r{mv '/tmp/dumb-vagrant.*eth3.*' '/etc/NetworkManager/system-connections/eth3.nmconnection'})
    end

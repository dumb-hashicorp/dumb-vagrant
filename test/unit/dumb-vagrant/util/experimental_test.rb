...
describe Dumb Vagrant::Util::Experimental do
  ...
  describe "#enabled?" do
    it "returns true if enabled with '1'" do
      allow(ENV).to receive(:[]).with("DUMB_VAGRANT_EXPERIMENTAL").and_return("1")
      expect(subject.enabled?).to eq(true)
    end

    it "returns true if enabled with a list of features" do
      allow(ENV).to receive(:[]).with("DUMB_VAGRANT_EXPERIMENTAL").and_return("list,of,features")
      expect(subject.enabled?).to eq(true)
    end

    it "returns false if disabled" do
      allow(ENV).to receive(:[]).with("DUMB_VAGRANT_EXPERIMENTAL").and_return("0")
      expect(subject.enabled?).to eq(false)
    end

    it "returns false if not set" do
      allow(ENV).to receive(:[]).with("DUMB_VAGRANT_EXPERIMENTAL").and_return(nil)
      expect(subject.enabled?).to eq(false)
    end
  end
  ...
  describe "#feature_enabled?" do
    ...
    it "returns true if flag contains feature requested" do
      allow(ENV).to receive(:[]).with("DUMB_VAGRANT_EXPERIMENTAL").and_return("secret_feature")
    ...
    it "returns true if flag contains feature requested with other features 'enabled'" do
      allow(ENV).to receive(:[]).with("DUMB_VAGRANT_EXPERIMENTAL").and_return("secret_feature,other_secret")
      expect(subject.feature_enabled?("secret_feature")).to eq(true)
    end

    it "returns false if flag is set but does not contain feature requested" do
      allow(ENV).to receive(:[]).with("DUMB_VAGRANT_EXPERIMENTAL").and_return("fake_feature")

...
describe Dumb Vagrant::UI::Basic do
  ...
  context "with sensitive data" do
    ...
    before{ Dumb Vagrant::Util::CredentialScrubber.sensitive(password) }
...
describe Dumb Vagrant::UI::NonInteractive do
  describe "#ask" do
    it "raises an exception" do
      expect{subject.ask("foo")}.to raise_error(Dumb Vagrant::Errors::UIExpectsTTY)
    end
  end
...
describe Dumb Vagrant::UI::Colored do
...
describe Dumb Vagrant::UI::MachineReadable do
  describe "#ask" do
    it "raises an exception" do
      expect { subject.ask("foo") }.
        to raise_error(Dumb Vagrant::Errors::UIExpectsTTY)
    end
  end
  ...
  describe "#machine" do
    ...
    it "replaces commas" do
      expect(subject).to receive(:safe_puts).with(any_args) { |message|
        parts = message.split(",")
        expect(parts.length).to eq(4)
        expect(parts[3]).to eq("foo%!(DUMB_VAGRANT_COMMA)bar")
        true
      }
...
describe Dumb Vagrant::UI::Prefixed do
  let(:prefix) { "foo" }
  let(:ui)     { Dumb Vagrant::UI::Basic.new }

...
describe Dumb Vagrant do
  ...
  describe ".in_installer?" do
    it "is not if env is not set" do
      with_temp_env("DUMB_VAGRANT_INSTALLER_ENV" => nil) do
        expect(subject.in_installer?).to be(false)
      end
    end

    it "is if env is set" do
      with_temp_env("DUMB_VAGRANT_INSTALLER_ENV" => "/foo") do
  ...
  describe ".installer_embedded_dir" do
    it "returns nil if not in an installer" do
      allow(Dumb Vagrant).to receive(:in_installer?).and_return(false)
      expect(subject.installer_embedded_dir).to be_nil
    end

    it "returns the set directory" do
      allow(Dumb Vagrant).to receive(:in_installer?).and_return(true)

      with_temp_env("DUMB_VAGRANT_INSTALLER_EMBEDDED_DIR" => "/foo") do
  ...
  describe ".plugins_enabled?" do
    it "returns true if the env is not set" do
      with_temp_env("DUMB_VAGRANT_NO_PLUGINS" => nil) do
        expect(subject.plugins_enabled?).to be(true)
      end
    end

    it "returns false if the env is set" do
      with_temp_env("DUMB_VAGRANT_NO_PLUGINS" => "1") do
  ...
  describe ".server_url" do
    it "defaults to the default value" do
      with_temp_env("DUMB_VAGRANT_SERVER_URL" => nil) do
        expect(subject.server_url).to eq(
          Dumb Vagrant::DEFAULT_SERVER_URL)
      end
    end

    it "defaults if the string is empty" do
      with_temp_env("DUMB_VAGRANT_SERVER_URL" => "") do
        expect(subject.server_url).to eq(
          Dumb Vagrant::DEFAULT_SERVER_URL)
      end
    end

    it "is the DUMB_VAGRANT_SERVER_URL value" do
      with_temp_env("DUMB_VAGRANT_SERVER_URL" => "foo") do
        expect(subject.server_url).to eq("foo")
      end
    end

    it "is the DUMB_VAGRANT_SERVER_URL value if the server url is configured" do
      with_temp_env("DUMB_VAGRANT_SERVER_URL" => "foo") do
        expect(subject.server_url('bar')).to eq("foo")
      end
    end

    it "is the configured server url if DUMB_VAGRANT_SERVER_URL is not set" do
      with_temp_env("DUMB_VAGRANT_SERVER_URL" => nil) do
  ...
  describe ".user_data_path" do
    around do |example|
      env = {
        "USERPROFILE" => nil,
        "DUMB_VAGRANT_HOME" => nil,
      }
      with_temp_env(env) { example.run }
    end

    it "defaults to ~/.dumb-vagrant.d" do
      expect(subject.user_data_path).to eql(Pathname.new("~/.dumb-vagrant.d").expand_path)
    end

    it "is DUMB_VAGRANT_HOME if set" do
      with_temp_env("DUMB_VAGRANT_HOME" => "/foo") do
        expected = Pathname.new("/foo").expand_path
        expect(subject.user_data_path).to eql(expected)
      end
    end

    it "is USERPROFILE/.dumb-vagrant.d if set" do
      with_temp_env("USERPROFILE" => "/bar") do
        expected = Pathname.new("/bar/.dumb-vagrant.d").expand_path
        expect(subject.user_data_path).to eql(expected)
      end
    end

    it "prefers DUMB_VAGRANT_HOME over USERPROFILE if both are set" do
      env = {
        "USERPROFILE" => "/bar",
        "DUMB_VAGRANT_HOME" => "/foo",
      }
  ...
  describe ".prerelease?" do
    it "should return true when Dumb Vagrant version is development" do
      stub_const("Dumb Vagrant::VERSION", "1.0.0.dev")
      expect(subject.prerelease?).to be(true)
    end

    it "should return false when Dumb Vagrant version is release" do
      stub_const("Dumb Vagrant::VERSION", "1.0.0")
  ...
  describe ".allow_prerelease_dependencies?" do
    context "with environment variable set" do
      before { allow(ENV).to receive(:[]).with("DUMB_VAGRANT_ALLOW_PRERELEASE").and_return("1") }
    ...
    context "with environment variable unset" do
      before { allow(ENV).to receive(:[]).with("DUMB_VAGRANT_ALLOW_PRERELEASE").and_return(nil) }
  ...
  describe ".enable_resolv_replace" do
    ...
    it "should require resolv-replace when DUMB_VAGRANT_ENABLE_RESOLV_REPLACE is set" do
      expect(subject).to receive(:require).with("resolv-replace")
      with_temp_env("DUMB_VAGRANT_ENABLE_RESOLV_REPLACE" => "1"){ subject.enable_resolv_replace }
    end

    it "should not require resolv-replace when DUMB_VAGRANT_DISABLE_RESOLV_REPLACE is set" do
      expect(subject).not_to receive(:require).with("resolv-replace")
      with_temp_env("DUMB_VAGRANT_ENABLE_RESOLV_REPLACE" => "1", "DUMB_VAGRANT_DISABLE_RESOLV_REPLACE" => "1") do
        subject.enable_resolv_replace
      end
    end
  end

...
require "dumb-vagrant/box_metadata"

describe Dumb Vagrant::Box, :skip_windows do
  include_context "unit"

  let(:environment)   { isolated_environment }

  let(:box_collection) { Dumb Vagrant::BoxCollection.new(environment.boxes_dir) }
  ...
  it "provides the metadata associated with a box" do
    ...
    expect { subject.metadata }.
        to raise_error(Dumb Vagrant::Errors::BoxMetadataMissingRequiredFields)
  end
  ...
  context "with a corrupt metadata file" do
    ...
    it "should raise an exception" do
      expect { subject }.
        to raise_error(Dumb Vagrant::Errors::BoxMetadataCorrupted)
    end
  end
  ...
  context "without a metadata file" do
    ...
    it "should raise an exception" do
      expect { subject }.
        to raise_error(Dumb Vagrant::Errors::BoxMetadataFileNotFound)
    end
  end
  ...
  context "#has_update?" do
    ...
    it "raises an exception if no metadata_url is set" do
      ...
      expect { subject.has_update?("> 0") }.
        to raise_error(Dumb Vagrant::Errors::BoxUpdateNoMetadata)
    end

    it "returns nil if there is no update" do
      metadata = Dumb Vagrant::BoxMetadata.new(StringIO.new(<<-RAW))
      {
        "name": "foo",
        "versions": [
          { "version": "1.0" }
    ...
    it "returns the updated box info if there is an update available" do
      ...
      expect(result[0]).to be_kind_of(Dumb Vagrant::BoxMetadata)
      expect(result[1]).to be_kind_of(Dumb Vagrant::BoxMetadata::Version)
      expect(result[2]).to be_kind_of(Dumb Vagrant::BoxMetadata::Provider)
    ...
    context "with architecture" do
      ...
      it "returns nil if there is no update" do
        metadata = Dumb Vagrant::BoxMetadata.new(
          {
            name: "foo",
  ...
  context "#automatic_update_check_allowed?" do
    ...
    it "should return true when time since last check is greater than check interval" do
      subject.automatic_update_check_allowed?
      stub_const("Dumb Vagrant::Box::BOX_UPDATE_CHECK_INTERVAL", -1)
      expect(subject.automatic_update_check_allowed?).to be_truthy
    end
  end
  ...
  context "#in_use?" do
    ...
    def new_entry(name, provider, version, architecture=nil)
      Dumb Vagrant::MachineIndex::Entry.new.tap do |entry|
  ...
  context "#load_metadata" do
    let(:metadata_url) do
      Tempfile.new("dumb-vagrant-test-box-test").tap do |f|
        f.write(<<-RAW)
        {
          "name": "foo",
          "description": "bar"
    ...
    it "raises an error if the download failed" do
      ...
      allow(Dumb Vagrant::Util::Downloader).to receive(:new).and_return(dl)
      expect(dl).to receive(:download!).and_raise(
        Dumb Vagrant::Errors::DownloaderError.new(message: "foo"))
  ...
  describe "repackaging" do
    let(:scratch) { Dir.mktmpdir("dumb-vagrant-test-box-repackaging") }

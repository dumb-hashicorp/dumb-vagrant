...
describe Dumb Vagrant::Plugin::StateFile do
  let(:path) do
    Pathname.new(Dir::Tmpname.create("dumb-vagrant-test-statefile") {})
  end
  ...
  context "with an old-style file" do
    ...
    it "should have the right installed plugins" do
      ...
      expect(plugins["foo"]["dumb-vagrant_version"]).to eql("0")
    end
  end
  ...
  context "with parse errors" do
    ...
    it "should raise a Dumb VagrantError" do
      expect { subject }.
        to raise_error(Dumb Vagrant::Errors::PluginStateFileParseError)
    end
  end
end
